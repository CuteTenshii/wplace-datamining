...
const Ee = () => "Latest",
  ...
  Ue = (n = {}, e = {}) => {
    const t = e.locale ?? fe();
    return t === "en" ? Ee() : t === "pt" ? Oe() : t === "ch" ? je() : t === "de" ? Be() : t === "es" ? De() : t === "fr" ? Fe() : t === "it" ? Ne() : t === "jp" ? He() : t === "pl" ? We() : t === "ru" ? Ze() : t === "uk" ? Qe() : Ge()
  },
  Ye = () => "Patch notes",
  ...
  An = (n = {}, e = {}) => {
    const t = e.locale ?? fe();
    return t === "en" ? Ye() : t === "pt" ? Xe() : t === "ch" ? Ke() : t === "de" ? Ve() : t === "es" ? Je() : t === "fr" ? et() : t === "it" ? tt() : t === "jp" ? nt() : t === "pl" ? rt() : t === "ru" ? st() : t === "uk" ? it() : at()
  };
  ...
  let t = typeof n == "string" ? n : n.source,
    s = {
      replace: (r, a) => {
        ...
        return l = l.replace(w.caret, "$1"), t = t.replace(r, l), s
      },
...
var lt = (() => {
  ...
  xe = f(be).replace(/bull/g, K).replace(/blockCode/g, /(?: {4}| {0,3}\t)/).replace(/fences/g, / {0,3}(?:`{3,}|~{3,})/).replace(/blockquote/g, / {0,3}>/).replace(/heading/g, / {0,3}#{1,6}/).replace(/html/g, / {0,3}<[^\n>]+>\n/).replace(/\|table/g, "").getRegex(),
  ut = f(be).replace(/bull/g, K).replace(/blockCode/g, /(?: {4}| {0,3}\t)/).replace(/fences/g, / {0,3}(?:`{3,}|~{3,})/).replace(/blockquote/g, / {0,3}>/).replace(/heading/g, / {0,3}#{1,6}/).replace(/html/g, / {0,3}<[^\n>]+>\n/).replace(/table/g, / {0,3}\|?(?:[:\- ]*\|)+[\:\- ]*\n/).getRegex(),
  ...
  ee = /<!--(?:-?>|[\s\S]*?(?:-->|$))/,
  mt = f("^ {0,3}(?:<(script|pre|style|textarea)[\\s>][\\s\\S]*?(?:</\\1>[^\\n]*\\n+|$)|comment[^\\n]*(\\n+|$)|<\\?[\\s\\S]*?(?:\\?>\\n*|$)|<![A-Z][\\s\\S]*?(?:>\\n*|$)|<!\\[CDATA\\[[\\s\\S]*?(?:\\]\\]>\\n*|$)|</?(tag)(?: +|\\n|/?>)[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$)|<(?!script|pre|style|textarea)([a-z][\\w-]*)(?:attribute)*? */?>(?=[ \\t]*(?:\\n|$))[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$)|</(?!script|pre|style|textarea)[a-z][\\w-]*\\s*>(?=[ \\t]*(?:\\n|$))[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$))", "i").replace("comment", ee).replace("tag", H).replace("attribute", / +[a-zA-Z:_][\w.:-]*(?: *= *"[^"\n]*"| *= *'[^'\n]*'| *= *[^\s"'=<>`]+)?/).getRegex(),
  we = f(V).replace("hr", L).replace("heading", " {0,3}#{1,6}(?:\\s|$)").replace("|lheading", "").replace("|table", "").replace("blockquote", " {0,3}>").replace("fences", " {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list", " {0,3}(?:[*+-]|1[.)])[ \\t]").replace("html", "</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag", H).getRegex(),
  ...
  bt = {
    ...
    lheading: ut,
    table: ce,
    paragraph: f(V).replace("hr", L).replace("heading", " {0,3}#{1,6}(?:\\s|$)").replace("|lheading", "").replace("table", ce).replace("blockquote", " {0,3}>").replace("fences", " {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list", " {0,3}(?:[*+-]|1[.)])[ \\t]").replace("html", "</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag", H).getRegex()
  },
  xt = {
    ...te,
  ...
  W = /[\s\p{P}\p{S}]/u,
  ne = /[^\s\p{P}\p{S}]/u,
  vt = f(/^((?![*_])punctSpace)/, "u").replace(/punctSpace/g, W).getRegex(),
  _e = /(?!~)[\p{P}\p{S}]/u,
  St = /(?!~)[\s\p{P}\p{S}]/u,
  Rt = /(?:[^\s\p{P}\p{S}]|~)/u,
  At = f(/link|precode-code|html/, "g").replace("link", /\[(?:[^\[\]`]|(?<a>`+)[^`]+\k<a>(?!`))*?\]\((?:\\[\s\S]|[^\\\(\)]|\((?:\\[\s\S]|[^\\\(\)])*\))*\)/).replace("precode-", lt ? "(?<!`)()" : "(^^|[^`])").replace("code", /(?<b>`+)[^`]+\k<b>(?!`)/).replace("html", /<(?! )[^<>]*?>/).getRegex(),
  ve = /^(?:\*+(?:((?!\*)punct)|([^\s*]))?)|^_+(?:((?!_)punct)|([^\s_]))?/,
  $t = f(ve, "u").replace(/punct/g, z).getRegex(),
  Tt = f(ve, "u").replace(/punct/g, _e).getRegex(),
  Se = "^[^_*]*?__[^_*]*?\\*[^_*]*?(?=__)|[^*]+(?=[^*])|(?!\\*)punct(\\*+)(?=[\\s]|$)|notPunctSpace(\\*+)(?!\\*)(?=punctSpace|$)|(?!\\*)punctSpace(\\*+)(?=notPunctSpace)|[\\s](\\*+)(?!\\*)(?=punct)|(?!\\*)punct(\\*+)(?!\\*)(?=punct)|notPunctSpace(\\*+)(?=notPunctSpace)",
  zt = f(Se, "gu").replace(/notPunctSpace/g, ne).replace(/punctSpace/g, W).replace(/punct/g, z).getRegex(),
  Ct = f(Se, "gu").replace(/notPunctSpace/g, Rt).replace(/punctSpace/g, St).replace(/punct/g, _e).getRegex(),
  Pt = f("^[^_*]*?\\*\\*[^_*]*?_[^_*]*?(?=\\*\\*)|[^_]+(?=[^_])|(?!_)punct(_+)(?=[\\s]|$)|notPunctSpace(_+)(?!_)(?=punctSpace|$)|(?!_)punctSpace(_+)(?=notPunctSpace)|[\\s](_+)(?!_)(?=punct)|(?!_)punct(_+)(?!_)(?=punct)", "gu").replace(/notPunctSpace/g, ne).replace(/punctSpace/g, W).replace(/punct/g, z).getRegex(),
  It = f(/^~~?(?:((?!~)punct)|[^\s~])/, "u").replace(/punct/g, z).getRegex(),
  qt = "^[^~]+(?=[^~])|(?!~)punct(~~?)(?=[\\s]|$)|notPunctSpace(~~?)(?!~)(?=punctSpace|$)|(?!~)punctSpace(~~?)(?=notPunctSpace)|[\\s](~~?)(?!~)(?=punct)|(?!~)punct(~~?)(?!~)(?=punct)|notPunctSpace(~~?)(?=notPunctSpace)",
  Mt = f(qt, "gu").replace(/notPunctSpace/g, ne).replace(/punctSpace/g, W).replace(/punct/g, z).getRegex(),
  Lt = f(/\\(punct)/, "gu").replace(/punct/g, z).getRegex(),
  Et = f(/^<(scheme:[^\s\x00-\x1f<>]*|email)>/).replace("scheme", /[a-zA-Z][a-zA-Z0-9+.-]{1,31}/).replace("email", /[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+(@)[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)+(?![-_])/).getRegex(),
  ...
  Re = f(/^!?\[(label)\]\[(ref)\]/).replace("label", D).replace("ref", J).getRegex(),
  Ae = f(/^!?\[(ref)\](?:\[\])?/).replace("ref", J).getRegex(),
  Dt = f("reflink|nolink(?!\\()", "g").replace("reflink", Re).replace("nolink", Ae).getRegex(),
  ...
  Ft = {
    ...re,
    link: f(/^!?\[(label)\]\((.*?)\)/).replace("label", D).getRegex(),
    reflink: f(/^!?\[(label)\]\s*\[([^\]]*)\]/).replace("label", D).getRegex()
  },
  G = {
    ...re,
...
function de(n, e) {
  var a;
  let t = n.replace(w.findPipe, (l, i, p) => {
      let o = !1,
        h = i;
      for (; --h >= 0 && p[h] === "\\";) o = !o;
      return o ? "|" : " |"
    ...
    r = 0;
  if (s[0].trim() || s.shift(), s.length > 0 && !((a = s.at(-1)) != null && a.trim()) && s.pop(), e)
...
`),
        ...
        for (; t.length > 0;) {
          ...
          for (p = 0; p < t.length; p++)
            if (this.rules.other.blockquoteStart.test(t[p])) i.push(t[p]), l = !0;
            else if (!l) i.push(t[p]);
          else break;
          t = t.slice(p);
          let o = i.join(`
`),
...
${h}` : h;
          ...
          if (this.lexer.state.top = !0, this.lexer.blockTokens(h, a, !0), this.lexer.state.top = d, t.length === 0) break;
          let u = a.at(-1);
...
`),
              g = this.blockquote(b);
            a[a.length - 1] = g, s = s.substring(0, s.length - c.raw.length) + g.raw, r = r.substring(0, r.length - c.text.length) + g.text;
            break
...
`),
              g = this.list(b);
            a[a.length - 1] = g, s = s.substring(0, s.length - u.raw.length) + g.raw, r = r.substring(0, r.length - c.raw.length) + g.raw, t = b.substring(a.at(-1).raw.length).split(`
`);
            continue
          }
        }
...
`, n = n.substring(b.length + 1), h = !0), !h) {
            let $ = this.rules.other.nextBulletRegex(x),
              E = this.rules.other.hrRegex(x),
              ie = this.rules.other.fencesBeginRegex(x),
              ae = this.rules.other.headingBeginRegex(x),
              $e = this.rules.other.htmlBeginRegex(x),
              Te = this.rules.other.blockquoteBeginRegex(x);
            for (; n;) {
              let Z = n.split(`
`, 1)[0],
                C;
              if (b = Z, this.options.pedantic ? (b = b.replace(this.rules.other.listReplaceNesting, "  "), C = b) : C = b.replace(this.rules.other.tabCharGlobal, "    "), ie.test(b) || ae.test(b) || $e.test(b) || Te.test(b) || $.test(b) || E.test(b)) break;
              if (C.search(this.rules.other.nonSpaceChar) >= x || !b.trim()) u += `
` + C.slice(x);
              else {
                if (g || c.replace(this.rules.other.tabCharGlobal, "    ").search(this.rules.other.nonSpaceChar) >= 4 || ie.test(c) || ae.test(c) || E.test(c)) break;
                u += `
` + b
              }
              g = !b.trim(), d += Z + `
`, n = n.substring(Z.length + 1), c = C.slice(x)
            }
          }
...
`) : [],
      ...
      if (t.length === s.length) {
        ...
        for (let i = 0; i < t.length; i++) a.header.push({
          text: t[i],
          tokens: this.lexer.inline(t[i]),
          header: !0,
          align: a.align[i]
        });
        for (let i of r) a.rows.push(de(i, a.header.length).map((p, o) => ({
          text: p,
...
` ? e[1].slice(0, -1) : e[1];
    ...
    url(n) {
      ...
      if (e = this.rules.inline.url.exec(n)) {
        ...
        if (e[2] === "@") s = e[0], r = "mailto:" + s;
          ...
          let a;
          do a = e[0], e[0] = ((t = this.rules.inline._backpedal.exec(e[0])) == null ? void 0 : t[0]) ?? ""; while (a !== e[0]);
  ...
  y = class U {
    constructor(e) {
      ...
      k(this, "inlineQueue");
      k(this, "tokenizer");
      this.tokens = [], this.tokens.links = Object.create(null), this.options = e || A, this.options.tokenizer = this.options.tokenizer || new F, this.tokenizer = this.options.tokenizer, this.tokenizer.options = this.options, this.tokenizer.lexer = this, this.inlineQueue = [], this.state = {
        inLink: !1,
        inRawBlock: !1,
        top: !0
      };
...
`), this.blockTokens(e, this.tokens);
    ...
    blockTokens(e, t = [], s = !1) {
      ...
      for (this.tokenizer.lexer = this, this.options.pedantic && (e = e.replace(w.tabCharGlobal, "    ").replace(w.spaceLine, "")); e;) {
        let i;
        if ((a = (r = this.options.extensions) == null ? void 0 : r.block) != null && a.some(o => (i = o.call({
            lexer: this
...
` + i.text, this.inlineQueue.pop(), this.inlineQueue.at(-1).src = o.text) : t.push(i);
    ...
    inlineTokens(e, t = []) {
      ...
      let a;
      for (;
        (r = this.tokenizer.rules.inline.blockSkip.exec(s)) !== null;) a = r[2] ? r[2].length : 0, s = s.slice(0, r.index + a) + "[" + "a".repeat(r[0].length - a - 2) + "]" + s.slice(this.tokenizer.rules.inline.blockSkip.lastIndex);
      ...
      for (; e;) {
        ...
        if ((u = this.options.extensions) != null && u.startInline) {
          ...
          this.options.extensions.startInline.forEach(E => {
            $ = E.call({
              lexer: this
            }, x), typeof $ == "number" && $ >= 0 && (g = Math.min(g, $))
          }), g < 1 / 0 && g >= 0 && (b = e.substring(0, g + 1))
        }
        if (c = this.tokenizer.inlineText(b)) {
          e = e.substring(c.raw.length), c.raw.slice(-1) !== "_" && (i = c.raw.slice(-1)), l = !0;
          let g = t.at(-1);
  ...
  N = class {
    ...
    code({
      ...
      var a;
      let s = (a = (e || "").match(w.notSpaceStart)) == null ? void 0 : a[0],
...
`
    ...
    image({
      ...
      n = r;
      let a = `<img src="${n}" alt="${v(t)}"`;
      return e && (a += ` title="${v(e)}"`), a += ">", a
    }
    text(n) {
  ...
  _ = class Y {
    constructor(e) {
      ...
      k(this, "renderer");
      k(this, "textRenderer");
      this.options = e || A, this.options.renderer = this.options.renderer || new N, this.renderer = this.options.renderer, this.renderer.options = this.options, this.renderer.parser = this, this.textRenderer = new se
    }
    static parse(e, t) {
  ...
  }, k(B, "passThroughHooks", new Set(["preprocess", "postprocess", "processAllTokens", "emStrongMask"])), k(B, "passThroughHooksRespectAsync", new Set(["preprocess", "postprocess", "processAllTokens"])), B),
  Gt = class {
    constructor(...n) {
      k(this, "defaults", X());
    ...
    walkTokens(n, e) {
      ...
      for (let a of n) switch (t = t.concat(e.call(this, a)), a.type) {
        case "table": {
          let l = a;
          for (let i of l.header) t = t.concat(this.walkTokens(i.tokens, e));
          for (let i of l.rows)
    ...
    use(...n) {
      ...
      return n.forEach(t => {
        ...
        if (t.tokenizer) {
          let r = this.defaults.tokenizer || new F(this.defaults);
        ...
        if (t.hooks) {
          ...
          for (let a in t.hooks) {
            ...
            let l = a,
              i = t.hooks[l],
              p = r[l];
            M.passThroughHooks.has(a) ? r[l] = o => {
              if (this.defaults.async && M.passThroughHooksRespectAsync.has(a)) return (async () => {
      ...
      return (e, t) => {
        ...
        if (r.hooks && (r.hooks.options = r, r.hooks.block = n), r.async) return (async () => {
          let l = r.hooks ? await r.hooks.preprocess(e) : e,
            i = await (r.hooks ? await r.hooks.provideLexer(n) : n ? y.lex : y.lexInline)(l, r),
        ...
        try {
          ...
          let l = (r.hooks ? r.hooks.provideLexer(n) : n ? y.lex : y.lexInline)(e, r);
          r.hooks && (l = r.hooks.processAllTokens(l)), r.walkTokens && this.walkTokens(l, r.walkTokens);
          let i = (r.hooks ? r.hooks.provideParser(n) : n ? _.parse : _.parseInline)(l, r);
...
`,
  ...
  Tn = Object.entries(fn).map(([n, e]) => {
    var a, l;
    const t = (a = n.split("/").at(-1)) == null ? void 0 : a.replace(".md", ""),
...
var mn = me('<span class="text-primary inline-flex items-center gap-1.5 text-[11px] font-semibold tracking-wide"><span class="relative flex size-1.5"><span class="bg-primary/60 absolute inline-flex size-full animate-ping rounded-full"></span> <span class="bg-primary relative inline-flex size-1.5 rounded-full"></span></span> </span>'),
  kn = me('<article><div class="flex items-center gap-3"><span class="text-primary text-lg font-bold tracking-wide"> </span> <!></div> <h2 class="mt-1 text-[0.9375rem] leading-snug font-bold tracking-tight"> </h2> <div class="patch-content mt-4 svelte-1pfqn89"></div></article>');

function zn(n, e) {
  Pe(e, !0);
  let t = Le(e, "latest", 3, !1);
  var s = kn(),
    r = P(s),
    a = P(r),
    l = P(a);
  ...
  T(o);
  var d = O(o, 2);
  Me(d, () => e.patchNote.html, !0), T(d), T(s), le(() => {
    Q(l, `v${e.patchNote.version??""}`), Q(h, e.patchNote.title)
  }), oe(n, s), Ie()
}
export {
  Gt as D, zn as P, $n as W, Tn as a, An as p, N as y
};