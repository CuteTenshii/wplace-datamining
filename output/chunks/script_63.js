...
(function() {
  try {
    ...
    e && (r._sentryDebugIds = r._sentryDebugIds || {}, r._sentryDebugIds[e] = "a410538a-fadf-491e-b3d4-d680f97f78b1", r._sentryDebugIdIdentifier = "sentry-dbid-a410538a-fadf-491e-b3d4-d680f97f78b1")
  } catch {}
})();
...
var Fe = (() => {
  ...
  We = f(ke).replace(/bull/g, X).replace(/blockCode/g, /(?: {4}| {0,3}\t)/).replace(/fences/g, / {0,3}(?:`{3,}|~{3,})/).replace(/blockquote/g, / {0,3}>/).replace(/heading/g, / {0,3}#{1,6}/).replace(/html/g, / {0,3}<[^\n>]+>\n/).replace(/table/g, / {0,3}\|?(?:[:\- ]*\|)+[\:\- ]*\n/).getRegex(),
  ...
  lt = f(Re, "u").replace(/punct/g, H).getRegex(),
  at = f(Re, "u").replace(/punct/g, ye).getRegex(),
  ve = "^[^_*]*?__[^_*]*?\\*[^_*]*?(?=__)|[^*]+(?=[^*])|(?!\\*)punct(\\*+)(?=[\\s]|$)|notPunctSpace(\\*+)(?!\\*)(?=punctSpace|$)|(?!\\*)punctSpace(\\*+)(?=notPunctSpace)|[\\s](\\*+)(?!\\*)(?=punct)|(?!\\*)punct(\\*+)(?!\\*)(?=punct)|notPunctSpace(\\*+)(?=notPunctSpace)",
  ot = f(ve, "gu").replace(/notPunctSpace/g, we).replace(/punctSpace/g, te).replace(/punct/g, H).getRegex(),
  ct = f(ve, "gu").replace(/notPunctSpace/g, rt).replace(/punctSpace/g, tt).replace(/punct/g, ye).getRegex(),
  ht = f("^[^_*]*?\\*\\*[^_*]*?_[^_*]*?(?=\\*\\*)|[^_]+(?=[^_])|(?!_)punct(_+)(?=[\\s]|$)|notPunctSpace(_+)(?!_)(?=punctSpace|$)|(?!_)punctSpace(_+)(?=notPunctSpace)|[\\s](_+)(?!_)(?=punct)|(?!_)punct(_+)(?!_)(?=punct)", "gu").replace(/notPunctSpace/g, we).replace(/punctSpace/g, te).replace(/punct/g, H).getRegex(),
  pt = f(/^~~?(?:((?!~)punct)|[^\s~])/, "u").replace(/punct/g, Se).getRegex(),
  ut = "^[^~]+(?=[^~])|(?!~)punct(~~?)(?=[\\s]|$)|notPunctSpace(~~?)(?!~)(?=punctSpace|$)|(?!~)punctSpace(~~?)(?=notPunctSpace)|[\\s](~~?)(?!~)(?=punct)|(?!~)punct(~~?)(?!~)(?=punct)|notPunctSpace(~~?)(?=notPunctSpace)",
  gt = f(ut, "gu").replace(/notPunctSpace/g, st).replace(/punctSpace/g, nt).replace(/punct/g, Se).getRegex(),
  dt = f(/\\(punct)/, "gu").replace(/punct/g, H).getRegex(),
  ...
  kt = f(K).replace("(?:-->|$)", "-->").getRegex(),
  xt = f("^comment|^</[a-zA-Z][\\w:-]*\\s*>|^<[a-zA-Z][\\w-]*(?:attribute)*?\\s*/?>|^<\\?[\\s\\S]*?\\?>|^<![a-zA-Z]+\\s[\\s\\S]*?>|^<!\\[CDATA\\[[\\s\\S]*?\\]\\]>").replace("comment", kt).replace("attribute", /\s+[a-zA-Z:_][\w.:-]*(?:\s*=\s*"[^"]*"|\s*=\s*'[^']*'|\s*=\s*[^\s"'=<>`]+)?/).getRegex(),
  F = /(?:\[(?:\\[\s\S]|[^\[\]\\])*\]|\\[\s\S]|`+[^`]*?`+(?!`)|[^\[\]\\`])*?/,
  bt = f(/^!?\[(label)\]\(\s*(href)(?:(?:[ \t]*(?:\n[ \t]*)?)(title))?\s*\)/).replace("label", F).replace("href", /<(?:\\.|[^\n<>\\])+>|[^ \t\n\x00-\x1f]*/).replace("title", /"(?:\\"?|[^"\\])*"|'(?:\\'?|[^'\\])*'|\((?:\\\)?|[^)\\])*\)/).getRegex(),
  $e = f(/^!?\[(label)\]\[(ref)\]/).replace("label", F).replace("ref", J).getRegex(),
  Te = f(/^!?\[(ref)\](?:\[\])?/).replace("ref", J).getRegex(),
  mt = f("reflink|nolink(?!\\()", "g").replace("reflink", $e).replace("nolink", Te).getRegex(),
...
var zt = de('<span class="badge badge-primary"> </span>'),
  At = de('<article class="card border-base-content/10 bg-base-100 border shadow-sm"><div class="card-body gap-4"><div class="flex items-center gap-3"><span class="badge badge-ghost"> </span> <!></div> <h2 class="text-xl font-semibold"> </h2> <div class="prose max-w-none"><!></div></div></article>');
...
export {
  Tt as B, Zt as P, Nt as V, Mt as p, M as y
};