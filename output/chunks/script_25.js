...
function qe(e = "", r = ur, o = gr, t = br) {
  const s = t(e),
    i = s % Ee * (360 / Ee);
  return [...Array(e ? 25 : 0)].reduce((l, p, c) => s & 1 << c % 15 ? l + `<rect x="${c>14?7-~~(c/5):~~(c/5)}" y="${c%5}" width="1" height="1"/>` : l, `<svg viewBox="-1.5 -1.5 8 8" xmlns="http://www.w3.org/2000/svg" fill="hsl(${i} ${r}% ${o}%)" shape-rendering="crispEdges">`) + "</svg>"
}
var je, A, oe, H, W, be, Fe;
(je = globalThis.customElements) != null && je.get("minidenticon-svg") || (Fe = globalThis.customElements) == null || Fe.define("minidenticon-svg", (A = class extends HTMLElement {
  constructor() {
    super(...arguments);
    Q(this, W);
    Q(this, H, !1)
...
}, oe = new WeakMap, H = new WeakMap, W = new WeakSet, be = function() {
  var s;
  const o = A.observedAttributes.map(i => this.getAttribute(i) || void 0),
    t = o.join(",");
  this.innerHTML = (s = ie(A, oe))[t] ?? (s[t] = qe(...o))
}, Pe(A, "observedAttributes", ["username", "saturation", "lightness"]), Q(A, oe, {}), A));
var fr = we("<div><!></div>");
...
const Oe = ["text-red-500", "text-orange-500", "text-yellow-500", "text-lime-500", "text-emerald-500", "text-teal-500", "text-cyan-500", "text-sky-500", "text-indigo-500", "text-violet-500", "text-purple-500", "text-fuchsia-500", "text-pink-500", "text-rose-500"],
  Ve = ["bg-red-500/10", "bg-orange-500/10", "bg-yellow-500/10", "bg-lime-500/10", "bg-emerald-500/10", "bg-teal-500/10", "bg-cyan-500/10", "bg-sky-500/10", "bg-indigo-500/10", "bg-violet-500/10", "bg-purple-500/10", "bg-fuchsia-500/10", "bg-pink-500/10", "bg-rose-500/10"];
...
function tr(e) {
  var r = e.r / 255,
    ...
    t = e.b / 255,
    s, i, l;
  return r = r > .04045 ? Math.pow((r + .055) / 1.055, 2.4) : r / 12.92, o = o > .04045 ? Math.pow((o + .055) / 1.055, 2.4) : o / 12.92, t = t > .04045 ? Math.pow((t + .055) / 1.055, 2.4) : t / 12.92, s = (r * .4124 + o * .3576 + t * .1805) / .95047, i = (r * .2126 + o * .7152 + t * .0722) / 1, l = (r * .0193 + o * .1192 + t * .9505) / 1.08883, s = s > .008856 ? Math.pow(s, 1 / 3) : 7.787 * s + 16 / 116, i = i > .008856 ? Math.pow(i, 1 / 3) : 7.787 * i + 16 / 116, l = l > .008856 ? Math.pow(l, 1 / 3) : 7.787 * l + 16 / 116, {
    l: 116 * i - 16,
    a: 500 * (s - i),
    b: 200 * (i - l)
  }
...
export {
  ho as A, co as P, hr as a, po as b, bo as c, fo as d, mo as g, go as h, uo as r, Kr as t
};