...
const Le = () => "Latest",
  ...
  Ue = (n = {}, e = {}) => {
    const t = e.locale ?? fe();
    return t === "en" ? Le() : t === "pt" ? Oe() : t === "ch" ? je() : t === "de" ? Fe() : t === "es" ? Be() : t === "fr" ? De() : t === "it" ? Ne() : t === "jp" ? He() : t === "pl" ? We() : t === "ru" ? Ze() : t === "uk" ? Qe() : Ge()
  },
  Ye = () => "Patch notes",
...
var lt = (() => {
  ...
  we = f(V).replace("hr", E).replace("heading", " {0,3}#{1,6}(?:\\s|$)").replace("|lheading", "").replace("|table", "").replace("blockquote", " {0,3}>").replace("fences", " {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list", " {0,3}(?:[*+-]|1[.)])[ \\t]").replace("html", "</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag", H).getRegex(),
  ...
  bt = {
    ...
    lheading: ut,
    table: ce,
    paragraph: f(V).replace("hr", E).replace("heading", " {0,3}#{1,6}(?:\\s|$)").replace("|lheading", "").replace("table", ce).replace("blockquote", " {0,3}>").replace("fences", " {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list", " {0,3}(?:[*+-]|1[.)])[ \\t]").replace("html", "</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag", H).getRegex()
  },
  xt = {
    ...te,
  ...
  Tt = f(ve, "u").replace(/punct/g, z).getRegex(),
  $t = f(ve, "u").replace(/punct/g, ye).getRegex(),
  ...
  Pt = f(Se, "gu").replace(/notPunctSpace/g, Rt).replace(/punctSpace/g, St).replace(/punct/g, ye).getRegex(),
  Ct = f("^[^_*]*?\\*\\*[^_*]*?_[^_*]*?(?=\\*\\*)|[^_]+(?=[^_])|(?!_)punct(_+)(?=[\\s]|$)|notPunctSpace(_+)(?!_)(?=punctSpace|$)|(?!_)punctSpace(_+)(?=notPunctSpace)|[\\s](_+)(?!_)(?=punct)|(?!_)punct(_+)(?!_)(?=punct)", "gu").replace(/notPunctSpace/g, ne).replace(/punctSpace/g, W).replace(/punct/g, z).getRegex(),
  ...
  Et = f(/\\(punct)/, "gu").replace(/punct/g, z).getRegex(),
  Lt = f(/^<(scheme:[^\s\x00-\x1f<>]*|email)>/).replace("scheme", /[a-zA-Z][a-zA-Z0-9+.-]{1,31}/).replace("email", /[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+(@)[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)+(?![-_])/).getRegex(),
  ...
  Re = f(/^!?\[(label)\]\[(ref)\]/).replace("label", B).replace("ref", J).getRegex(),
  Ae = f(/^!?\[(ref)\](?:\[\])?/).replace("ref", J).getRegex(),
  Bt = f("reflink|nolink(?!\\()", "g").replace("reflink", Re).replace("nolink", Ae).getRegex(),
...
function de(n, e) {
  var i;
  let t = n.replace(w.findPipe, (l, a, p) => {
      let o = !1,
        h = a;
      for (; --h >= 0 && p[h] === "\\";) o = !o;
      return o ? "|" : " |"
...
`, n = n.substring(b.length + 1), h = !0), !h) {
            let T = this.rules.other.nextBulletRegex(x),
              L = this.rules.other.hrRegex(x),
              ae = this.rules.other.fencesBeginRegex(x),
              ie = this.rules.other.headingBeginRegex(x),
              Te = this.rules.other.htmlBeginRegex(x),
              $e = this.rules.other.blockquoteBeginRegex(x);
            for (; n;) {
              let Z = n.split(`
`, 1)[0],
                P;
...
` + a.text, this.inlineQueue.pop(), this.inlineQueue.at(-1).src = o.text) : t.push(a);
  ...
  N = class {
    ...
    code({
      ...
      var i;
      let s = (i = (e || "").match(w.notSpaceStart)) == null ? void 0 : i[0],
...
`,
  ...
  xn = Object.assign({
    ...
    "./markdown/1.4.1 - 🛡️ Alliance System Fixes.md": mn,
    "./markdown/1.4.2 - 🛠️ Headquarters Preview Reliability.md": bn
  }),
  In = Object.entries(xn).map(([n, e]) => {
    var i, l;
...
var wn = me('<span class="text-primary inline-flex items-center gap-1.5 text-[11px] font-semibold tracking-wide"><span class="relative flex size-1.5"><span class="bg-primary/60 absolute inline-flex size-full animate-ping rounded-full"></span> <span class="bg-primary relative inline-flex size-1.5 rounded-full"></span></span> </span>'),
  _n = me('<article><div class="flex items-center gap-3"><span class="text-primary text-lg font-bold tracking-wide"> </span> <!></div> <h2 class="mt-1 text-[0.9375rem] leading-snug font-bold tracking-tight"> </h2> <div class="patch-content mt-4 svelte-1pfqn89"></div></article>');
...
export {
  Gt as D, qn as P, Cn as W, In as a, Pn as p, N as y
};