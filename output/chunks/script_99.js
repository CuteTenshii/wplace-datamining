...
const Le = () => "Latest",
  ...
  Ge = (n = {}, e = {}) => {
    const t = e.locale ?? me();
    return t === "en" ? Le() : t === "pt" ? Oe() : t === "ch" ? je() : t === "de" ? Fe() : t === "es" ? Be() : t === "fr" ? De() : t === "it" ? He() : t === "jp" ? Ne() : t === "pl" ? We() : t === "ru" ? Ze() : t === "uk" ? Qe() : Ue()
  },
  Ye = () => "Patch notes",
  ...
  at = () => "Нотатки про оновлення",
  it = () => "Ghi chú cập nhật",
  In = (n = {}, e = {}) => {
    const t = e.locale ?? me();
    return t === "en" ? Ye() : t === "pt" ? Xe() : t === "ch" ? Ke() : t === "de" ? Ve() : t === "es" ? Je() : t === "fr" ? et() : t === "it" ? tt() : t === "jp" ? nt() : t === "pl" ? rt() : t === "ru" ? st() : t === "uk" ? at() : it()
  };
...
var lt = (() => {
  ...
  we = m(be).replace(/bull/g, K).replace(/blockCode/g, /(?: {4}| {0,3}\t)/).replace(/fences/g, / {0,3}(?:`{3,}|~{3,})/).replace(/blockquote/g, / {0,3}>/).replace(/heading/g, / {0,3}#{1,6}/).replace(/html/g, / {0,3}<[^\n>]+>\n/).replace(/\|table/g, "").getRegex(),
  ut = m(be).replace(/bull/g, K).replace(/blockCode/g, /(?: {4}| {0,3}\t)/).replace(/fences/g, / {0,3}(?:`{3,}|~{3,})/).replace(/blockquote/g, / {0,3}>/).replace(/heading/g, / {0,3}#{1,6}/).replace(/html/g, / {0,3}<[^\n>]+>\n/).replace(/table/g, / {0,3}\|?(?:[:\- ]*\|)+[\:\- ]*\n/).getRegex(),
  ...
  xe = m(V).replace("hr", E).replace("heading", " {0,3}#{1,6}(?:\\s|$)").replace("|lheading", "").replace("|table", "").replace("blockquote", " {0,3}>").replace("fences", " {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list", " {0,3}(?:[*+-]|1[.)])[ \\t]").replace("html", "</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag", N).getRegex(),
  ...
  bt = {
    ...
    lheading: ut,
    table: ce,
    paragraph: m(V).replace("hr", E).replace("heading", " {0,3}#{1,6}(?:\\s|$)").replace("|lheading", "").replace("table", ce).replace("blockquote", " {0,3}>").replace("fences", " {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list", " {0,3}(?:[*+-]|1[.)])[ \\t]").replace("html", "</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag", N).getRegex()
  },
  wt = {
    ...te,
    ...
    fences: S,
    lheading: /^(.+?)\n {0,3}(=+|-+) *(?:\n+|$)/,
    paragraph: m(V).replace("hr", E).replace("heading", ` *#{1,6} *[^
  ...
  St = /(?!~)[\s\p{P}\p{S}]/u,
  Rt = /(?:[^\s\p{P}\p{S}]|~)/u,
  At = m(/link|precode-code|html/, "g").replace("link", /\[(?:[^\[\]`]|(?<a>`+)[^`]+\k<a>(?!`))*?\]\((?:\\[\s\S]|[^\\\(\)]|\((?:\\[\s\S]|[^\\\(\)])*\))*\)/).replace("precode-", lt ? "(?<!`)()" : "(^^|[^`])").replace("code", /(?<b>`+)[^`]+\k<b>(?!`)/).replace("html", /<(?! )[^<>]*?>/).getRegex(),
  ve = /^(?:\*+(?:((?!\*)punct)|([^\s*]))?)|^_+(?:((?!_)punct)|([^\s_]))?/,
  Tt = m(ve, "u").replace(/punct/g, z).getRegex(),
  $t = m(ve, "u").replace(/punct/g, ye).getRegex(),
  Se = "^[^_*]*?__[^_*]*?\\*[^_*]*?(?=__)|[^*]+(?=[^*])|(?!\\*)punct(\\*+)(?=[\\s]|$)|notPunctSpace(\\*+)(?!\\*)(?=punctSpace|$)|(?!\\*)punctSpace(\\*+)(?=notPunctSpace)|[\\s](\\*+)(?!\\*)(?=punct)|(?!\\*)punct(\\*+)(?!\\*)(?=punct)|notPunctSpace(\\*+)(?=notPunctSpace)",
  zt = m(Se, "gu").replace(/notPunctSpace/g, ne).replace(/punctSpace/g, W).replace(/punct/g, z).getRegex(),
  Pt = m(Se, "gu").replace(/notPunctSpace/g, Rt).replace(/punctSpace/g, St).replace(/punct/g, ye).getRegex(),
  Ct = m("^[^_*]*?\\*\\*[^_*]*?_[^_*]*?(?=\\*\\*)|[^_]+(?=[^_])|(?!_)punct(_+)(?=[\\s]|$)|notPunctSpace(_+)(?!_)(?=punctSpace|$)|(?!_)punctSpace(_+)(?=notPunctSpace)|[\\s](_+)(?!_)(?=punct)|(?!_)punct(_+)(?!_)(?=punct)", "gu").replace(/notPunctSpace/g, ne).replace(/punctSpace/g, W).replace(/punct/g, z).getRegex(),
  It = m(/^~~?(?:((?!~)punct)|[^\s~])/, "u").replace(/punct/g, z).getRegex(),
  qt = "^[^~]+(?=[^~])|(?!~)punct(~~?)(?=[\\s]|$)|notPunctSpace(~~?)(?!~)(?=punctSpace|$)|(?!~)punctSpace(~~?)(?=notPunctSpace)|[\\s](~~?)(?!~)(?=punct)|(?!~)punct(~~?)(?!~)(?=punct)|notPunctSpace(~~?)(?=notPunctSpace)",
  Mt = m(qt, "gu").replace(/notPunctSpace/g, ne).replace(/punctSpace/g, W).replace(/punct/g, z).getRegex(),
  Et = m(/\\(punct)/, "gu").replace(/punct/g, z).getRegex(),
  Lt = m(/^<(scheme:[^\s\x00-\x1f<>]*|email)>/).replace("scheme", /[a-zA-Z][a-zA-Z0-9+.-]{1,31}/).replace("email", /[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+(@)[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)+(?![-_])/).getRegex(),
  Ot = m(ee).replace("(?:-->|$)", "-->").getRegex(),
  jt = m("^comment|^</[a-zA-Z][\\w:-]*\\s*>|^<[a-zA-Z][\\w-]*(?:attribute)*?\\s*/?>|^<\\?[\\s\\S]*?\\?>|^<![a-zA-Z]+\\s[\\s\\S]*?>|^<!\\[CDATA\\[[\\s\\S]*?\\]\\]>").replace("comment", Ot).replace("attribute", /\s+[a-zA-Z:_][\w.:-]*(?:\s*=\s*"[^"]*"|\s*=\s*'[^']*'|\s*=\s*[^\s"'=<>`]+)?/).getRegex(),
  B = /(?:\[(?:\\[\s\S]|[^\[\]\\])*\]|\\[\s\S]|`+(?!`)[^`]*?`+(?!`)|``+(?=\])|[^\[\]\\`])*?/,
  Ft = m(/^!?\[(label)\]\(\s*(href)(?:(?:[ \t]+(?:\n[ \t]*)?|\n[ \t]*)(title))?\s*\)/).replace("label", B).replace("href", /<(?:\\.|[^\n<>\\])+>|[^ \t\n\x00-\x1f]*/).replace("title", /"(?:\\"?|[^"\\])*"|'(?:\\'?|[^'\\])*'|\((?:\\\)?|[^)\\])*\)/).getRegex(),
  Re = m(/^!?\[(label)\]\[(ref)\]/).replace("label", B).replace("ref", J).getRegex(),
  Ae = m(/^!?\[(ref)\](?:\[\])?/).replace("ref", J).getRegex(),
  Bt = m("reflink|nolink(?!\\()", "g").replace("reflink", Re).replace("nolink", Ae).getRegex(),
...
function v(n, e) {
  if (e) {
    if (x.escapeTest.test(n)) return n.replace(x.escapeReplace, he)
  } else if (x.escapeTestNoEncode.test(n)) return n.replace(x.escapeReplaceNoEncode, he);
  return n
}
...
function de(n, e) {
  var i;
  let t = n.replace(x.findPipe, (l, a, p) => {
      let o = !1,
        h = a;
      for (; --h >= 0 && p[h] === "\\";) o = !o;
      return o ? "|" : " |"
  ...
  for (; r < s.length; r++) s[r] = s[r].trim().replace(x.slashPipe, "|");
  return s
}
    ...
    code(n) {
      ...
      if (e) {
        ...
        return {
          ...
          text: this.options.pedantic ? t : q(t, `
`)
        }
      }
    }
...
`),
        ...
        for (; t.length > 0;) {
          ...
          for (p = 0; p < t.length; p++)
            if (this.rules.other.blockquoteStart.test(t[p])) a.push(t[p]), l = !0;
            else if (!l) a.push(t[p]);
          else break;
          t = t.slice(p);
          let o = a.join(`
`),
...
`),
              g = this.list(b);
            i[i.length - 1] = g, s = s.substring(0, s.length - u.raw.length) + g.raw, r = r.substring(0, r.length - c.raw.length) + g.raw, t = b.substring(i.at(-1).raw.length).split(`
`);
            continue
          }
        }
...
`, 1)[0],
            g = !c.trim(),
            w = 0;
          if (this.options.pedantic ? (w = 2, u = c.trimStart()) : g ? w = e[1].length + 1 : (w = c.search(this.rules.other.nonSpaceChar), w = w > 4 ? 1 : w, u = c.slice(w), w += e[1].length), g && this.rules.other.blankLine.test(b) && (d += b + `
`, n = n.substring(b.length + 1), h = !0), !h) {
            let T = this.rules.other.nextBulletRegex(w),
              L = this.rules.other.hrRegex(w),
              ae = this.rules.other.fencesBeginRegex(w),
              ie = this.rules.other.headingBeginRegex(w),
              Te = this.rules.other.htmlBeginRegex(w),
              $e = this.rules.other.blockquoteBeginRegex(w);
            for (; n;) {
              let Z = n.split(`
`, 1)[0],
                P;
              if (b = Z, this.options.pedantic ? (b = b.replace(this.rules.other.listReplaceNesting, "  "), P = b) : P = b.replace(this.rules.other.tabCharGlobal, "    "), ae.test(b) || ie.test(b) || Te.test(b) || $e.test(b) || T.test(b) || L.test(b)) break;
              if (P.search(this.rules.other.nonSpaceChar) >= w || !b.trim()) u += `
` + P.slice(w);
              else {
                if (g || c.replace(this.rules.other.tabCharGlobal, "    ").search(this.rules.other.nonSpaceChar) >= 4 || ae.test(c) || ie.test(c) || L.test(c)) break;
                u += `
` + b
              }
              g = !b.trim(), d += Z + `
`, n = n.substring(Z.length + 1), c = P.slice(w)
            }
          }
          l.loose || (p ? l.loose = !0 : this.rules.other.doubleBlankLine.test(d) && (p = !0)), l.items.push({
            type: "list_item",
            raw: d,
        ...
        for (let h of l.items) {
          if (this.lexer.state.top = !1, h.tokens = this.lexer.blockTokens(h.text, []), h.task) {
            ...
            if (d) {
              ...
              h.checked = u.checked, l.loose ? h.tokens[0] && ["paragraph", "text"].includes(h.tokens[0].type) && "tokens" in h.tokens[0] && h.tokens[0].tokens ? (h.tokens[0].raw = u.raw + h.tokens[0].raw, h.tokens[0].text = u.raw + h.tokens[0].text, h.tokens[0].tokens.unshift(u)) : h.tokens.unshift({
                type: "paragraph",
                raw: u.raw,
                text: u.raw,
                tokens: [u]
    ...
    table(n) {
      ...
      let t = de(e[1]),
        ...
        r = (l = e[3]) != null && l.trim() ? e[3].replace(this.rules.other.tableRowBlankLine, "").split(`
`) : [],
        i = {
          type: "table",
          raw: e[0],
      ...
      if (t.length === s.length) {
        ...
        for (let a = 0; a < t.length; a++) i.header.push({
          text: t[a],
          tokens: this.lexer.inline(t[a]),
          header: !0,
          align: i.align[a]
        });
        for (let a of r) i.rows.push(de(a, i.header.length).map((p, o) => ({
          text: p,
...
` ? e[1].slice(0, -1) : e[1];
    ...
    del(n, e, t = "") {
      ...
      if (s && (!s[1] || !t || this.rules.inline.punctuation.exec(t))) {
        ...
        for (p.lastIndex = 0, e = e.slice(-1 * n.length + r);
          (s = p.exec(e)) !== null;) {
          if (i = s[1] || s[2] || s[3] || s[4] || s[5] || s[6], !i || (l = [...i].length, l !== r)) continue;
          if (s[3] || s[4]) {
            a += l;
            continue
          }
  ...
  _ = class G {
    constructor(e) {
      ...
      k(this, "inlineQueue");
      k(this, "tokenizer");
      this.tokens = [], this.tokens.links = Object.create(null), this.options = e || A, this.options.tokenizer = this.options.tokenizer || new D, this.tokenizer = this.options.tokenizer, this.tokenizer.options = this.options, this.tokenizer.lexer = this, this.inlineQueue = [], this.state = {
        inLink: !1,
        inRawBlock: !1,
        top: !0
      };
...
`), this.blockTokens(e, this.tokens);
    ...
    blockTokens(e, t = [], s = !1) {
      ...
      for (this.tokenizer.lexer = this, this.options.pedantic && (e = e.replace(x.tabCharGlobal, "    ").replace(x.spaceLine, "")); e;) {
        let a;
        if ((i = (r = this.options.extensions) == null ? void 0 : r.block) != null && i.some(o => (a = o.call({
            lexer: this
...
`) ? "" : `
`) + a.raw, o.text += `
` + a.text, this.inlineQueue.pop(), this.inlineQueue.at(-1).src = o.text) : t.push(a), s = p.length !== e.length, e = e.substring(a.raw.length);
          continue
        }
...
` + a.text, this.inlineQueue.pop(), this.inlineQueue.at(-1).src = o.text) : t.push(a);
    ...
    inlineTokens(e, t = []) {
      ...
      let i;
      for (;
        (r = this.tokenizer.rules.inline.blockSkip.exec(s)) !== null;) i = r[2] ? r[2].length : 0, s = s.slice(0, r.index + i) + "[" + "a".repeat(r[0].length - i - 2) + "]" + s.slice(this.tokenizer.rules.inline.blockSkip.lastIndex);
      ...
      for (; e;) {
        ...
        if ((u = this.options.extensions) != null && u.startInline) {
          ...
          this.options.extensions.startInline.forEach(L => {
            T = L.call({
              lexer: this
            }, w), typeof T == "number" && T >= 0 && (g = Math.min(g, T))
  ...
  H = class {
    ...
    code({
      ...
      var i;
      let s = (i = (e || "").match(x.notSpaceStart)) == null ? void 0 : i[0],
        r = n.replace(x.endingNewline, "") + `
`;
...
`
  ...
  y = class Y {
    constructor(e) {
      ...
      k(this, "renderer");
      k(this, "textRenderer");
      this.options = e || A, this.options.renderer = this.options.renderer || new H, this.renderer = this.options.renderer, this.renderer.options = this.options, this.renderer.parser = this, this.textRenderer = new se
    }
    static parse(e, t) {
    ...
    parseInline(e, t = this.renderer) {
      ...
      for (let l = 0; l < e.length; l++) {
        ...
        if ((i = (r = this.options.extensions) == null ? void 0 : r.renderers) != null && i[a.type]) {
          let o = this.options.extensions.renderers[a.type].call({
            parser: this
          }, a);
  ...
  F, M = (F = class {
    ...
    provideLexer(n = this.block) {
      return n ? _.lex : _.lexInline
    }
    provideParser(n = this.block) {
      return n ? y.parse : y.parseInline
    }
  }, k(F, "passThroughHooks", new Set(["preprocess", "postprocess", "processAllTokens", "emStrongMask"])), k(F, "passThroughHooksRespectAsync", new Set(["preprocess", "postprocess", "processAllTokens"])), F),
  Ut = class {
    constructor(...n) {
      k(this, "defaults", X());
    ...
    walkTokens(n, e) {
      ...
      for (let i of n) switch (t = t.concat(e.call(this, i)), i.type) {
        case "table": {
          let l = i;
          for (let a of l.header) t = t.concat(this.walkTokens(a.tokens, e));
          for (let a of l.rows)
            for (let p of a) t = t.concat(this.walkTokens(p.tokens, e));
          break
        }
        case "list": {
          let l = i;
        ...
        default: {
          let l = i;
          (r = (s = this.defaults.extensions) == null ? void 0 : s.childTokens) != null && r[l.type] ? this.defaults.extensions.childTokens[l.type].forEach(a => {
            let p = l[a].flat(1 / 0);
        ...
        if (t.hooks) {
          ...
          for (let i in t.hooks) {
            ...
            let l = i,
              a = t.hooks[l],
              p = r[l];
            M.passThroughHooks.has(i) ? r[l] = o => {
    ...
    parseMarkdown(n) {
      return (e, t) => {
        ...
        if (r.hooks && (r.hooks.options = r, r.hooks.block = n), r.async) return (async () => {
          let l = r.hooks ? await r.hooks.preprocess(e) : e,
            a = await (r.hooks ? await r.hooks.provideLexer(n) : n ? _.lex : _.lexInline)(l, r),
            p = r.hooks ? await r.hooks.processAllTokens(a) : a;
          r.walkTokens && await Promise.all(this.walkTokens(p, r.walkTokens));
          let o = await (r.hooks ? await r.hooks.provideParser(n) : n ? y.parse : y.parseInline)(p, r);
          return r.hooks ? await r.hooks.postprocess(o) : o
        })().catch(i);
        try {
          r.hooks && (e = r.hooks.preprocess(e));
          let l = (r.hooks ? r.hooks.provideLexer(n) : n ? _.lex : _.lexInline)(e, r);
          r.hooks && (l = r.hooks.processAllTokens(l)), r.walkTokens && this.walkTokens(l, r.walkTokens);
          let a = (r.hooks ? r.hooks.provideParser(n) : n ? y.parse : y.parseInline)(l, r);
...
f.options = f.setOptions = function(n) {
  return R.setOptions(n), f.defaults = R.defaults, ke(f.defaults), f
};
f.getDefaults = X;
f.defaults = A;
f.use = function(...n) {
  return R.use(...n), f.defaults = R.defaults, ke(f.defaults), f
};
...
`,
  ...
  Vt = `This update brings several improvements and fixes for a smoother experience:
...
`,
  ...
  _n = Object.assign({
    ...
    "./markdown/1.3.0 - 🛠️ Moderation Adjustments, Hotspots and Improved Anti-Cheat System.md": an,
    ...
    "./markdown/1.4.2 - 🛠️ Headquarters Preview Reliability.md": bn,
    "./markdown/1.4.3 - 🛠️ Alliance Usability Fixes.md": xn
  }),
  Mn = Object.entries(_n).map(([n, e]) => {
    var i, l;
    const t = (i = n.split("/").at(-1)) == null ? void 0 : i.replace(".md", ""),
      s = (l = t == null ? void 0 : t.split("-")[0]) == null ? void 0 : l.trim(),
...
var yn = fe('<span class="text-primary inline-flex items-center gap-1.5 text-[11px] font-semibold tracking-wide"><span class="relative flex size-1.5"><span class="bg-primary/60 absolute inline-flex size-full animate-ping rounded-full"></span> <span class="bg-primary relative inline-flex size-1.5 rounded-full"></span></span> </span>'),
  vn = fe('<article><div class="flex items-center gap-3"><span class="text-primary text-lg font-bold tracking-wide"> </span> <!></div> <h2 class="mt-1 text-[0.9375rem] leading-snug font-bold tracking-tight"> </h2> <div class="patch-content mt-4 svelte-1pfqn89"></div></article>');
...
function En(n, e) {
  ...
  $(o);
  var d = O(o, 2);
  Me(d, () => e.patchNote.html, !0), $(d), $(s), le(() => {
    Q(l, `v${e.patchNote.version??""}`), Q(h, e.patchNote.title)
  }), oe(n, s), Ie()
}
export {
  Ut as D, En as P, qn as W, Mn as a, In as p, H as y
};