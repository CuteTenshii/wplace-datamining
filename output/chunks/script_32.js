...
var Ee = (l, a, s, o, _) => {
    b(a).show(), C(s, !0), Ot.generatePixQrCode(o()).then(p => {
      C(_, p, !0)
    }).catch(p => {
      J.error(p.message)
    }).finally(() => {
  ...
  Xe = async (l, a, s) => {
    ...
    try {
      ...
      const {
        paid: c
      } = await Ot.getPixStatus(b(a).pixId);
      if (c) {
...
function da(l, a) {
  ...
  var jt = r(vt, 2);
  {
    var Kt = d => {
        ...
        var X = r(E, 2),
          tt = t(X);
        tt.__click = [Oe, c], e(X), e(O), e(F);
        var j = r(F, 2),
          K = t(j);
        K.__click = [Xe, c, f], e(j), z(et => {
