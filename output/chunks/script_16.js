...
const pn = ui(),
  ...
  Qr = me && "flat" in Array.prototype && !("ReportingObserver" in globalThis);
...
function wr({
  ...
  let _ = {
    ...
    "failed class extends error": !mt && be({
      spawnErr: () => {
        class f extends n {}
      }
    }),
    ...
    "failed object toString error": be({
      ...
      withStack: f => me && !st(f, yi)
    }) || be({
      spawnErr: () => {
        Object.create(new Proxy(n, {})).toString()
      },
      withStack: f => me && !st(f, wi)
    }),
  ...
  if (a == "toString" || !!o["Function.toString"] || !!o["Permissions.query"]) {
    ...
    _ = {
      ...
      "failed at reflect set proto": ht && be({
        spawnErr: () => {
      ...
      "failed at reflect set proto proxy": ht && !be({
        spawnErr: () => {
      ...
      "failed at instanceof check error": me && (be({
        spawnErr: () => {
          n instanceof n
        },
        withStack: h => !st(h, bi)
      ...
      "failed at define properties": me && ht && gi(() => {
...
async function Gi() {
  try {
    ...
    const v = me && CSS.supports("accent-color: initial");
    ((P, L) => {
...
async function Vi() {
  ...
  try {
    ...
    const C = {
        ...
        "AnalyserNode.context.listener.forwardX.maxValue": k(() => Se(() => p.context.listener.forwardX.maxValue)),
      ...
      E = F => new Promise(j => {
        const $ = F.createAnalyser(),
          ...
          de = F.createDynamicsCompressor();
        try {
          ae.type = "triangle", ae.frequency.value = 1e4, de.threshold.value = -50, de.knee.value = 40, de.attack.value = 0
        } catch {}
        return ae.connect(de), de.connect($), de.connect(F.destination), ae.start(0), F.startRendering(), F.addEventListener("complete", _e => {
          var Fe;
          try {
            de.disconnect(), ae.disconnect();
    ...
    const T = (F, j, $) => {
        const ae = Math.min($, F.length),
          de = [];
        for (let _e = j; _e < ae; _e++) de.push(F[_e]);
        return de
      },
      I = F => {
        if (!F) return 0;
    ...
    Ee && !Oe && (c = !0, te("AudioBuffer", "getChannelData and copyFromChannel samples mismatch"));
    ...
    const xe = (F, j) => Math.floor(Math.random() * (j - F + 1)) + F,
      De = (F, j, $) => {
        const {
          length: ae
        } = j, de = 20, _e = xe(275, ae - (de + 1)), Fe = _e + de / 2, Le = _e + de;
        j.getChannelData(0)[_e] = F, j.getChannelData(0)[Fe] = F, j.getChannelData(0)[Le] = F, j.copyFromChannel($, 0);
        const Ke = [j.getChannelData(0)[_e] === 0 ? Math.random() : 0, j.getChannelData(0)[Fe] === 0 ? Math.random() : 0, j.getChannelData(0)[Le] === 0 ? Math.random() : 0];
        return [...new Set([...j.getChannelData(0), ...$, ...Ke])].filter(Jt => Jt !== 0)
      },
      at = (F, j, $) => {
        j.copyToChannel($.map(() => F), 0);
        const ae = j.getChannelData(0)[0];
        return [...j.getChannelData(0)].map(_e => _e !== ae || !_e ? Math.random() : _e).filter(_e => _e !== ae)
      };
    let W = (() => {
      try {
    ...
    const oe = "" + [v, B, P],
      se = ce[oe];
    return se && !se.includes(G) && (Pe.AUDIO = !0, ue("AudioBuffer", "suspicious frequency data")), {
...
async function qi() {
  ...
  try {
    ...
    const E = new Set,
      D = oo.reduce((L, U) => {
        const {
          ...
          width: xe
        } = f.measureText(U) || {}, De = [ee, we, G, Oe, Ee, ve, xe].join(",");
    ...
    ze.imageDataLowEntropy = I, (me && !B.BLINK.includes(I) || St && !B.GECKO.includes(I) || mt && !B.WEBKIT.includes(I)) && (Pe.CANVAS = !0), Pe.CANVAS && ue("CanvasRenderingContext2D.getImageData", "suspicious pixel data");
    const P = L => {
      const U = De => De % 1 !== 0,
        {
          actualBoundingBoxAscent: ee,
      ...
      return [ee, we, G, Oe, Ee, ve].find(De => U(De || 0))
    };
...
function ls() {
  ...
  const t = "getVideoPlaybackQuality" in HTMLVideoElement.prototype,
    ...
    E = (T, I) => T ? [I] : [],
    D = {
      [ye.ANDROID]: [...E(o, s), ...E(e, _), ...E(t, u), c, ...E(i, !p), ...E(r, !f), ...E(a, !d), ...E(a, !g), !h, y, ...E(n, !C)],
      [ye.CHROME_OS]: [...E(o, s), ...E(e, !_), ...E(t, !u), c, ...E(i, p), ...E(r, f), ...E(a, d), ...E(a, g), h, y || !y, ...E(n, !C)],
      [ye.WINDOWS]: [...E(o, !s), ...E(e, !_), ...E(t, !u), !c, ...E(i, p), ...E(r, f), ...E(a, d), ...E(a, g), h, y || !y, ...E(n, C)],
      [ye.MAC]: [...E(o, s), ...E(e, !_), ...E(t, !u), !c, ...E(i, p), ...E(r, f), ...E(a, d), ...E(a, g), h, !y, ...E(n, C)],
      [ye.LINUX]: [...E(o, !s), ...E(e, !_), ...E(t, !u), !c, ...E(i, p), ...E(r, f), ...E(a, d), ...E(a, g), h, !y || !y, ...E(n, !C)]
    },
    S = {
      noContentIndex: e && !_,
  ...
  try {
    ...
    const r = qe(),
      ...
      u = {
        chromium: me,
        likeHeadless: {
          noChrome: me && !("chrome" in window),
          hasPermissionsBug: me && "permissions" in navigator && await (async () => (await navigator.permissions.query({
            name: "notifications"
          })).state == "prompt" && "Notification" in window && Notification.permission === "denied")(),
          noPlugins: me && navigator.plugins.length === 0,
          noMimeTypes: me && a.length === 0,
          notificationIsDenied: me && "Notification" in window && Notification.permission == "denied",
          hasKnownBgColor: me && (() => {
            let S = o;
          ...
          noWebShare: me && CSS.supports("accent-color: initial") && (!("share" in navigator) || !("canShare" in navigator)),
...
async function fs() {
  ...
  try {
    ...
    const n = qe(),
      ...
      i = Se(() => new Intl.NumberFormat(void 0, {
        notation: "compact",
        compactDisplay: "long"
      }).format(21e6)),
      s = Se(() => new Intl.PluralRules().select(1)),
...
function vs(t, n) {
  if (n) {
    ...
    if (n >= 2 && En.contentDocumentHash.includes(t.contentDocumentHash) && En.contentWindowHash.includes(t.contentDocumentHash) && En.getContextHash.includes(t.getContextHash) && t.hardwareConcurrencyHash == fe) return "NoScript";
  ...
  try {
    ...
    const t = Ri(),
      n = {
        ...
        mode: void 0,
        extension: void 0,
        engine: me ? "Blink" : St ? "Gecko" : ""
      },
      [e, r] = await Promise.all([Zr(), me ? void 0 : Es()]);
    if (e) {
      const c = li();
    ...
    const i = {
      ...
      s = {};
    for (const [c, _] of Object.entries(i)) _ !== fe && (s[c.slice(0, -4)] = _);
...
async function xs() {
  ...
  try {
    ...
    const Oe = fi((o = U.parameters) == null ? void 0 : o.UNMASKED_RENDERER_WEBGL),
      Ee = "" + G,
      ve = !Oe || !Ee ? void 0 : K([Oe, Ee]),
      xe = G ? G.reduce((x, H, W) => x ^ +H + W, 0) : void 0;
    ze.webglParams = Ee, ze.webglBrandCapabilities = ve, ze.webglCapabilities = xe;
    const De = ve && !ee.includes(ve),
      at = xe && !we.includes(xe);
...
async function ks() {
  var n, e, r, o, a, i, s, c, _, u, p, f, d, g, h, y, C, E, D, S, N, A, w, v, T, I, B, P, L, U, ee, we;
  const t = await Ps();
  ...
  return {
    ...
    intl: !t.intl || t.intl.lied ? void 0 : {
      ...
      relativeTimeFormat: (we = t.intl) == null ? void 0 : we.relativeTimeFormat
    },
...
async function Ps() {
  try {
    const [t, n, e, r, o, a, i, s, c, _, u, p, f, d] = await Promise.all([ge("offlineAudioContext", Vi()), ge("canvasWebgl", xs()), ge("canvas2d", qi()), ge("css", Promise.resolve(Ki())), ge("cssMedia", Promise.resolve(Xi())), ge("screen", As()), ge("consoleErrors", Promise.resolve(Ji())), ge("timezone", Promise.resolve(Os())), ge("fonts", ss()), ge("media", ms()), ge("resistance", Ts()), ge("intl", fs()), ge("workerScope", Gi()), ge("maths", Promise.resolve(ii()))]), [g, h, y, C, E] = await Promise.all([ge("navigator", hs(f)), ge("headless", _s({
      webgl: n
    })), ge("lies", Promise.resolve(Di())), ge("trash", Promise.resolve(ki())), ge("capturedErrors", Promise.resolve(ci()))]), [D, S] = await Promise.all([fr((n == null ? void 0 : n.pixels) ?? []), fr((n == null ? void 0 : n.pixels2) ?? [])]);
    return {
      navigator: g,
      headless: h,
      cssMedia: o,
...
async function Ls() {
  ...
  return Qt = (async () => {
    var a, i, s, c, _, u, p, f, d, g, h, y, C, E, D, S, N, A, w, v, T, I, B, P, L, U, ee, we, G, Oe, Ee, ve, xe, De, at, x, H, W, ce, oe, se, F, j, $, ae, de, _e, Fe, Le, Ke, Jt, $n, qn, Kn, Xn, Yn, Jn, Qn, Zn, er, tr, nr, rr;
    const t = await ks();
...
class Ms {
  constructor() {
    ...
    Q(this, Nt, pe(null));
    Q(this, Ct, pe(!1));
    Q(this, xt, pe("custom-winter"));
    Q(this, kt, pe(an(Date.now())));
    Q(this, Pt, pe(void 0));
    setInterval(() => {
