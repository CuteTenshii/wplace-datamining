...
var _e = (r, e, t) => e in r ? Pe(r, e, {
  enumerable: !0,
  configurable: !0,
  writable: !0,
  value: t
}) : r[e] = t;
var x = (r, e, t) => _e(r, typeof e != "symbol" ? e + "" : e, t);
import "./D4N6Bqe8.js";
import {
  p as Ie,
  d as $,
...
(function() {
  try {
    ...
    e && (r._sentryDebugIds = r._sentryDebugIds || {}, r._sentryDebugIds[e] = "de7b6a05-fdc9-477e-9489-8c887061ecd7", r._sentryDebugIdIdentifier = "sentry-dbid-de7b6a05-fdc9-477e-9489-8c887061ecd7")
  } catch {}
})();
...
var Me = (() => {
  ...
  J = /<!--(?:-?>|[\s\S]*?(?:-->|$))/,
  Ge = f("^ {0,3}(?:<(script|pre|style|textarea)[\\s>][\\s\\S]*?(?:</\\1>[^\\n]*\\n+|$)|comment[^\\n]*(\\n+|$)|<\\?[\\s\\S]*?(?:\\?>\\n*|$)|<![A-Z][\\s\\S]*?(?:>\\n*|$)|<!\\[CDATA\\[[\\s\\S]*?(?:\\]\\]>\\n*|$)|</?(tag)(?: +|\\n|/?>)[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$)|<(?!script|pre|style|textarea)([a-z][\\w-]*)(?:attribute)*? */?>(?=[ \\t]*(?:\\n|$))[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$)|</(?!script|pre|style|textarea)[a-z][\\w-]*\\s*>(?=[ \\t]*(?:\\n|$))[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$))", "i").replace("comment", J).replace("tag", Z).replace("attribute", / +[a-zA-Z:_][\w.:-]*(?: *= *"[^"\n]*"| *= *'[^'\n]*'| *= *[^\s"'=<>`]+)?/).getRegex(),
  xe = f(V).replace("hr", L).replace("heading", " {0,3}#{1,6}(?:\\s|$)").replace("|lheading", "").replace("|table", "").replace("blockquote", " {0,3}>").replace("fences", " {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list", " {0,3}(?:[*+-]|1[.)])[ \\t]").replace("html", "</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag", Z).getRegex(),
  ...
  te = /[\s\p{P}\p{S}]/u,
  we = /[^\s\p{P}\p{S}]/u,
  et = f(/^((?![*_])punctSpace)/, "u").replace(/punctSpace/g, te).getRegex(),
  ye = /(?!~)[\p{P}\p{S}]/u,
  tt = /(?!~)[\s\p{P}\p{S}]/u,
  rt = /(?:[^\s\p{P}\p{S}]|~)/u,
  ve = /(?![*_])[\p{P}\p{S}]/u,
  nt = /(?![*_])[\s\p{P}\p{S}]/u,
  st = /(?:[^\s\p{P}\p{S}]|[*_])/u,
  it = f(/link|precode-code|html/, "g").replace("link", /\[(?:[^\[\]`]|(?<a>`+)[^`]+\k<a>(?!`))*?\]\((?:\\[\s\S]|[^\\\(\)]|\((?:\\[\s\S]|[^\\\(\)])*\))*\)/).replace("precode-", Me ? "(?<!`)()" : "(^^|[^`])").replace("code", /(?<b>`+)[^`]+\k<b>(?!`)/).replace("html", /<(?! )[^<>]*?>/).getRegex(),
  Se = /^(?:\*+(?:((?!\*)punct)|[^\s*]))|^_+(?:((?!_)punct)|([^\s_]))/,
  lt = f(Se, "u").replace(/punct/g, W).getRegex(),
  at = f(Se, "u").replace(/punct/g, ye).getRegex(),
  Re = "^[^_*]*?__[^_*]*?\\*[^_*]*?(?=__)|[^*]+(?=[^*])|(?!\\*)punct(\\*+)(?=[\\s]|$)|notPunctSpace(\\*+)(?!\\*)(?=punctSpace|$)|(?!\\*)punctSpace(\\*+)(?=notPunctSpace)|[\\s](\\*+)(?!\\*)(?=punct)|(?!\\*)punct(\\*+)(?!\\*)(?=punct)|notPunctSpace(\\*+)(?=notPunctSpace)",
  ot = f(Re, "gu").replace(/notPunctSpace/g, we).replace(/punctSpace/g, te).replace(/punct/g, W).getRegex(),
  ct = f(Re, "gu").replace(/notPunctSpace/g, rt).replace(/punctSpace/g, tt).replace(/punct/g, ye).getRegex(),
  ht = f("^[^_*]*?\\*\\*[^_*]*?_[^_*]*?(?=\\*\\*)|[^_]+(?=[^_])|(?!_)punct(_+)(?=[\\s]|$)|notPunctSpace(_+)(?!_)(?=punctSpace|$)|(?!_)punctSpace(_+)(?=notPunctSpace)|[\\s](_+)(?!_)(?=punct)|(?!_)punct(_+)(?!_)(?=punct)", "gu").replace(/notPunctSpace/g, we).replace(/punctSpace/g, te).replace(/punct/g, W).getRegex(),
  pt = f(/^~~?(?:((?!~)punct)|[^\s~])/, "u").replace(/punct/g, ve).getRegex(),
  ut = "^[^~]+(?=[^~])|(?!~)punct(~~?)(?=[\\s]|$)|notPunctSpace(~~?)(?!~)(?=punctSpace|$)|(?!~)punctSpace(~~?)(?=notPunctSpace)|[\\s](~~?)(?!~)(?=punct)|(?!~)punct(~~?)(?!~)(?=punct)|notPunctSpace(~~?)(?=notPunctSpace)",
  dt = f(ut, "gu").replace(/notPunctSpace/g, st).replace(/punctSpace/g, nt).replace(/punct/g, ve).getRegex(),
  gt = f(/\\(punct)/, "gu").replace(/punct/g, W).getRegex(),
  ft = f(/^<(scheme:[^\s\x00-\x1f<>]*|email)>/).replace("scheme", /[a-zA-Z][a-zA-Z0-9+.-]{1,31}/).replace("email", /[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+(@)[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)+(?![-_])/).getRegex(),
  kt = f(J).replace("(?:-->|$)", "-->").getRegex(),
  mt = f("^comment|^</[a-zA-Z][\\w:-]*\\s*>|^<[a-zA-Z][\\w-]*(?:attribute)*?\\s*/?>|^<\\?[\\s\\S]*?\\?>|^<![a-zA-Z]+\\s[\\s\\S]*?>|^<!\\[CDATA\\[[\\s\\S]*?\\]\\]>").replace("comment", kt).replace("attribute", /\s+[a-zA-Z:_][\w.:-]*(?:\s*=\s*"[^"]*"|\s*=\s*'[^']*'|\s*=\s*[^\s"'=<>`]+)?/).getRegex(),
  M = /(?:\[(?:\\[\s\S]|[^\[\]\\])*\]|\\[\s\S]|`+[^`]*?`+(?!`)|[^\[\]\\`])*?/,
  xt = f(/^!?\[(label)\]\(\s*(href)(?:(?:[ \t]*(?:\n[ \t]*)?)(title))?\s*\)/).replace("label", M).replace("href", /<(?:\\.|[^\n<>\\])+>|[^ \t\n\x00-\x1f]*/).replace("title", /"(?:\\"?|[^"\\])*"|'(?:\\'?|[^'\\])*'|\((?:\\\)?|[^)\\])*\)/).getRegex(),
  $e = f(/^!?\[(label)\]\[(ref)\]/).replace("label", M).replace("ref", K).getRegex(),
  Te = f(/^!?\[(ref)\](?:\[\])?/).replace("ref", K).getRegex(),
  bt = f("reflink|nolink(?!\\()", "g").replace("reflink", $e).replace("nolink", Te).getRegex(),
  ...
  wt = {
    ...re,
    link: f(/^!?\[(label)\]\((.*?)\)/).replace("label", M).getRegex(),
    reflink: f(/^!?\[(label)\]\s*\[([^\]]*)\]/).replace("label", M).getRegex()
  },
  j = {
    ...re,
...
`, 1)[0],
            g = !c.trim(),
            b = 0;
          if (this.options.pedantic ? (b = 2, u = c.trimStart()) : g ? b = e[1].length + 1 : (b = c.search(this.rules.other.nonSpaceChar), b = b > 4 ? 1 : b, u = c.slice(b), b += e[1].length), g && this.rules.other.blankLine.test(k) && (d += k + `
`, r = r.substring(k.length + 1), p = !0), !p) {
            let R = this.rules.other.nextBulletRegex(b),
              B = this.rules.other.hrRegex(b),
              se = this.rules.other.fencesBeginRegex(b),
              ie = this.rules.other.headingBeginRegex(b),
              Ae = this.rules.other.htmlBeginRegex(b),
              ze = this.rules.other.blockquoteBeginRegex(b);
            for (; r;) {
              let H = r.split(`
`, 1)[0],
                _;
              if (k = H, this.options.pedantic ? (k = k.replace(this.rules.other.listReplaceNesting, "  "), _ = k) : _ = k.replace(this.rules.other.tabCharGlobal, "    "), se.test(k) || ie.test(k) || Ae.test(k) || ze.test(k) || R.test(k) || B.test(k)) break;
              if (_.search(this.rules.other.nonSpaceChar) >= b || !k.trim()) u += `
` + _.slice(b);
              else {
...
` + k
              ...
              g = !k.trim(), d += H + `
`, r = r.substring(H.length + 1), c = _.slice(b)
            }
          }
...
` ? e[1].slice(0, -1) : e[1];
  ...
  y = class G {
    constructor(e) {
      ...
      x(this, "inlineQueue");
      x(this, "tokenizer");
      this.tokens = [], this.tokens.links = Object.create(null), this.options = e || P, this.options.tokenizer = this.options.tokenizer || new F, this.tokenizer = this.options.tokenizer, this.tokenizer.options = this.options, this.tokenizer.lexer = this, this.inlineQueue = [], this.state = {
        inLink: !1,
        inRawBlock: !1,
        top: !0
      };
      ...
      for (; e;) {
        ...
        if ((u = this.options.extensions) != null && u.startInline) {
          ...
          this.options.extensions.startInline.forEach(B => {
            R = B.call({
              lexer: this
            }, b), typeof R == "number" && R >= 0 && (g = Math.min(g, R))
...
`
    ...
    image({
      ...
      let n = pe(r);
      if (n === null) return S(t);
      r = n;
      let l = `<img src="${r}" alt="${S(t)}"`;
  ...
  v = class U {
    constructor(e) {
      ...
      x(this, "renderer");
      x(this, "textRenderer");
      this.options = e || P, this.options.renderer = this.options.renderer || new N, this.renderer = this.options.renderer, this.renderer.options = this.options, this.renderer.parser = this, this.textRenderer = new ne
    }
    static parse(e, t) {
  ...
  }, x(D, "passThroughHooks", new Set(["preprocess", "postprocess", "processAllTokens", "emStrongMask"])), x(D, "passThroughHooksRespectAsync", new Set(["preprocess", "postprocess", "processAllTokens"])), D),
  Tt = class {
    constructor(...r) {
      x(this, "defaults", Y());
    ...
    use(...r) {
      ...
      return r.forEach(t => {
        ...
        if (s.async = this.defaults.async || s.async || !1, t.extensions && (t.extensions.forEach(n => {
          ...
          let n = this.defaults.renderer || new N(this.defaults);
        ...
        if (t.hooks) {
          let n = this.defaults.hooks || new E;
          for (let l in t.hooks) {
            ...
            let a = l,
              i = t.hooks[a],
              h = n[a];
            E.passThroughHooks.has(l) ? n[a] = o => {
    ...
    parseMarkdown(r) {
      return (e, t) => {
        ...
        if (n.hooks && (n.hooks.options = n, n.hooks.block = r), n.async) return (async () => {
          ...
          let o = await (n.hooks ? await n.hooks.provideParser() : r ? v.parse : v.parseInline)(h, n);
        ...
        try {
          ...
          let i = (n.hooks ? n.hooks.provideParser() : r ? v.parse : v.parseInline)(a, n);
...
Please report this to https://github.com/markedjs/marked.`, r) {
          let s = "<p>An error occurred:</p><pre>" + S(t.message + "", !0) + "</pre>";
...
m.options = m.setOptions = function(r) {
  return z.setOptions(r), m.defaults = z.defaults, fe(m.defaults), m
};
m.getDefaults = Y;
m.defaults = P;
m.use = function(...r) {
  return z.use(...r), m.defaults = z.defaults, fe(m.defaults), m
};
m.walkTokens = function(r, e) {
  return z.walkTokens(r, e)
};
m.parseInline = z.parseInline;
m.Parser = v;
m.parser = v.parse;
m.Renderer = N;
m.TextRenderer = ne;
...
- Fixed a stray canvas click when confirming a profile picture`
  ...
  version: "1.3.0",
  title: "🛠️ Moderation Adjustments, Hotspots and Improved Anti-Cheat System",
  markdown: `
With v1.3.0, we’re reducing the moderation intervention and giving more freedom to the players to better match Wplace’s core idea: a living canvas shaped by the community.
...
var At = ge('<span class="badge badge-primary"> </span>'),
  zt = ge('<article class="card border-base-content/10 bg-base-100 border shadow-sm"><div class="card-body gap-4"><div class="flex items-center gap-3"><span class="badge badge-ghost"> </span> <!></div> <h2 class="text-xl font-semibold"> </h2> <div class="prose max-w-none"><!></div></div></article>');
...
function Zt(r, e) {
  ...
  var u = O(p, 2),
    c = $(u);
  Le(c, () => e.patchNote.html), T(u), T(n), T(s), le(() => {
    Q(i, `v${e.patchNote.version??""}`), Be(p, "id", e.patchNote.version), Q(d, e.patchNote.title)
  }), ae(r, s), Ce()
}
export {
  Tt as B, Zt as P, Ft as V, Nt as p, N as y
};