...
const Ft = () => "and",
  ...
  te = () => "继续即表示你同意我们的",
  ee = () => "Wenn du fortfährst, stimmst du unseren",
  ne = () => "Al continuar, aceptas nuestra",
  re = () => "En continuant, vous acceptez nos",
  ie = () => "Continuando, accetti i nostri",
  oe = () => "続行すると、以下に同意したものとみなされます:",
  ue = () => "Kontynuując, akceptujesz nasze",
  ce = () => "Продолжая, вы соглашаетесь с нашими",
  ae = () => "Продовжуючи, ви погоджуєтеся з нашими",
  se = () => "Khi tiếp tục, bạn đồng ý với",
  _e = (e = {}, r = {}) => {
    const t = r.locale ?? p();
    return t === "en" ? Qt() : t === "pt" ? Xt() : t === "ch" ? te() : t === "de" ? ee() : t === "es" ? ne() : t === "fr" ? re() : t === "it" ? ie() : t === "jp" ? oe() : t === "pl" ? ue() : t === "ru" ? ce() : t === "uk" ? ae() : se()
  },
  le = () => "Community Guidelines",
  pe = () => "Diretrizes da Comunidade",
  fe = () => "社区准则",
  de = () => "Community-Richtlinien",
  ve = () => "Normas de la Comunidad",
  we = () => "Règles de la communauté",
  ge = () => "Linee guida della community",
  me = () => "コミュニティガイドライン",
  he = () => "Wytyczne Społeczności",
  be = () => "Правила сообщества",
  ye = () => "Правила спільноти",
  xe = () => "Nguyên tắc Cộng đồng",
  ke = (e = {}, r = {}) => {
    const t = r.locale ?? p();
    return t === "en" ? le() : t === "pt" ? pe() : t === "ch" ? fe() : t === "de" ? de() : t === "es" ? ve() : t === "fr" ? we() : t === "it" ? ge() : t === "jp" ? me() : t === "pl" ? he() : t === "ru" ? be() : t === "uk" ? ye() : xe()
  },
  ze = () => "Have a Wplace Twitch account?",
  je = () => "Você tem uma conta Wplace Twitch?",
  Ce = () => "你有 Wplace Twitch 账号吗？",
  Te = () => "Hast du ein Wplace-Twitch-Konto?",
  We = () => "¿Tienes una cuenta de Wplace Twitch?",
  Me = () => "Vous avez un compte Wplace Twitch ?",
  $e = () => "Hai un account Wplace Twitch?",
  Ae = () => "WplaceのTwitchアカウントをお持ちですか？",
  Be = () => "Masz konto Wplace Twitch?",
  Le = () => "Есть аккаунт Wplace Twitch?",
  Pe = () => "Є акаунт Wplace Twitch?",
  Se = () => "Bạn có tài khoản Wplace Twitch không?",
  Ie = (e = {}, r = {}) => {
    const t = r.locale ?? p();
  ...
  tn = () => "Inicia sesión para colocar píxeles en el mapa del mundo compartido.",
  en = () => "Connectez-vous pour placer des pixels sur la carte du monde partagée.",
  nn = () => "Accedi per posizionare pixel sulla mappa del mondo condivisa.",
  rn = () => "ログインして、みんなの世界地図にピクセルを置こう。",
  on = () => "Zaloguj się, aby umieszczać piksele na wspólnej mapie świata.",
  un = () => "Войдите, чтобы размещать пиксели на общей карте мира.",
  cn = () => "Увійдіть, щоб розміщувати пікселі на спільній карті світу.",
  ...
  fn = () => "Konto migrieren",
  dn = () => "Migra tu cuenta",
  vn = () => "Migrer votre compte",
  wn = () => "Migra il tuo account",
  gn = () => "アカウントを移行",
  mn = () => "Przenieś swoje konto",
  hn = () => "Мигрировать аккаунт",
  bn = () => "Перенести акаунт",
  yn = () => "Di chuyển tài khoản",
  xn = (e = {}, r = {}) => {
    const t = r.locale ?? p();
    return t === "en" ? _n() : t === "pt" ? ln() : t === "ch" ? pn() : t === "de" ? fn() : t === "es" ? dn() : t === "fr" ? vn() : t === "it" ? wn() : t === "jp" ? gn() : t === "pl" ? mn() : t === "ru" ? hn() : t === "uk" ? bn() : yn()
  },
  kn = () => "Open in browser",
  zn = () => "Abrir no navegador",
  jn = () => "在浏览器中打开",
  Cn = () => "Im Browser öffnen",
  Tn = () => "Abrir en el navegador",
  Wn = () => "Ouvrir dans le navigateur",
  Mn = () => "Apri nel browser",
  $n = () => "ブラウザで開く",
  An = () => "Otwórz w przeglądarce",
  Bn = () => "Открыть в браузере",
  Ln = () => "Відкрити у браузері",
  Pn = () => "Mở trong trình duyệt",
  Sn = (e = {}, r = {}) => {
    const t = r.locale ?? p();
    return t === "en" ? kn() : t === "pt" ? zn() : t === "ch" ? jn() : t === "de" ? Cn() : t === "es" ? Tn() : t === "fr" ? Wn() : t === "it" ? Mn() : t === "jp" ? $n() : t === "pl" ? An() : t === "ru" ? Bn() : t === "uk" ? Ln() : Pn()
  },
  In = () => "Privacy Policy",
  ...
  er = () => "Termini di servizio",
  nr = () => "利用規約",
  rr = () => "Warunki korzystania z usługi",
  ir = () => "Условия обслуживания",
  or = () => "Умови обслуговування",
  ...
  _r = () => "部分功能可能无法正常工作。建议使用常用浏览器访问。",
  lr = () => "Einige Funktionen funktionieren möglicherweise nicht richtig. Wir empfehlen, die Seite über einen gängigen Browser aufzurufen.",
  pr = () => "Algunas funcionalidades podrían no funcionar correctamente. Recomendamos acceder mediante un navegador común.",
  fr = () => "Certaines fonctionnalités pourraient ne pas fonctionner correctement. Nous vous recommandons d'accéder via un navigateur standard.",
  dr = () => "Alcune funzionalità potrebbero non funzionare correttamente. Si consiglia di accedere tramite un browser comune.",
  vr = () => "いくつかの機能が正常に動作しない場合があります。通常のブラウザからアクセスすることをお勧めします。",
  wr = () => "Niektóre funkcje mogą nie działać poprawnie. Zalecamy korzystanie ze zwykłej przeglądarki.",
  gr = () => "Некоторые функции могут работать неправильно. Мы рекомендуем использовать обычный браузер.",
  mr = () => "Деякі функції можуть працювати некоректно. Рекомендуємо використовувати звичайний браузер.",
  hr = () => "Một số tính năng có thể không hoạt động đúng cách. Chúng tôi khuyên bạn nên truy cập bằng trình duyệt thông thường.",
  br = (e = {}, r = {}) => {
    const t = r.locale ?? p();
    return t === "en" ? ar() : t === "pt" ? sr() : t === "ch" ? _r() : t === "de" ? lr() : t === "es" ? pr() : t === "fr" ? fr() : t === "it" ? dr() : t === "jp" ? vr() : t === "pl" ? wr() : t === "ru" ? gr() : t === "uk" ? mr() : hr()
  },
  yr = () => "You are in a webview",
  xr = () => "Você está em uma webview",
  kr = () => "您正在使用网页视图",
  zr = () => "Sie befinden sich in einer Webansicht",
  jr = () => "Estás en una webview",
  Cr = () => "Vous êtes dans une webview",
  Tr = () => "Sei in una webview",
  Wr = () => "あなたはウェブビュー内にいます",
  Mr = () => "Jesteś w widoku internetowym",
  $r = () => "Вы находитесь в веб-просмотре",
  Ar = () => "Ви перебуваєте у веб-перегляді",
  Br = () => "Bạn đang ở trong một webview",
  Lr = (e = {}, r = {}) => {
    const t = r.locale ?? p();
