...
const W = () => "Unexpected server error. Try again later.",
  ...
  Ce = (t = {}, e = {}) => (e.locale ?? s()) === "en" ? Me() : Ie(),
  Ae = () => "You do not have enough droplets to buy this item.",
  Oe = () => "Você não tem gotas suficientes para comprar este item.",
  ke = (t = {}, e = {}) => (e.locale ?? s()) === "en" ? Ae() : Oe(),
...
class Qa {
  ...
  async paint(e, a) {
    const n = Fa(e, r => `t=(${r.tile[0]},${r.tile[1]}),s=${r.season}`),
      i = (await Promise.all(Object.values(n).map(r => {
        const [c, _] = r[0].tile, S = r[0].season, J = {
          colors: r.map(N => N.colorIdx),
          coords: r.flatMap(N => N.pixel),
          t: a
        };
        return this.request(`/s${S}/pixel/${c}/${_}`, {
          method: "POST",
    ...
    if (i.length) {
      ...
      if (r.status === 403) {
        ...
        c == null || c.err;
        const _ = c == null ? void 0 : c.suspension;
  ...
  async createAlliance(e) {
    ...
    if (a.status === 400) {
      const n = await a.json();
      throw n.error === "max_characters" ? new Error(Ge()) : n.error === "name_taken" ? new Error(Re()) : n.error == "empty_name" ? new Error(Ke()) : new Error(o())
  ...
  async getModeratorTickets() {
    ...
    if (e.status !== 200) throw new g(o(), e.status);
    const a = await e.json();
    for (const n of a.tickets) n.reports.sort((l, i) => R[l.reason] - R[i.reason]);
    return a
  }
  ...
  async setTicketStatus(e, a) {
    ...
    if (n.status !== 200) throw new g(o(), n.status)
  }
  async request(e, a) {
    let n;
    try {
  ...
  async getUserInfoById(e) {
    ...
    if (a.status !== 404) {
      if (a.status !== 200) throw new g(o(), a.status);
      return a.json()
    }
  }
...
class et {
  ...
  set(e, a) {
    ...
    if (n >= this.bytes.length) {
      ...
      for (let _ = 0; _ < this.bytes.length; _++) r[_ + c] = this.bytes[_];
      this.bytes = r
    }
...
class St {
  constructor() {
    m(this, D, B(() => f.toasts.filter(e => !e.dismiss)))
  }
  get toasts() {
    return h(d(this, D))
  }
...
const ot = rt,
  st = Object.assign(ot, {
    ...
    dismiss: f.dismiss,
    loading: f.loading,
    getActiveToasts: () => f.toasts.filter(t => !t.dismiss)
  });
var M, I, C, A, O, k, x;
class it {
  constructor() {
...
M = new WeakMap, I = new WeakMap, C = new WeakMap, A = new WeakMap, O = new WeakMap, k = new WeakMap, x = new WeakMap;
const lt = new it;
export {
  $ as C, gt as P, Wa as S, j as a, o as b, z as c, f as d, bt as e, St as f, K as g, ht as h, ma as i, wa as j, Ta as k, ha as l, Sa as m, yt as n, pt as o, wt as p, mt as q, ft as r, _t as s, st as t, lt as u
};