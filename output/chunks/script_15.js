...
const Bt = "https://backend.wplace.live/files",
  Ot = "0x4AAAAAABpHqZ-6i7uL0nmG",
  Y = "https://backend.wplace.live",
  Dt = "0x4AAAAAABpqJe8FO0N84q0F";
let F = q({
...
const W = () => "Unexpected server error. Try again later.",
  ...
  Ja = () => "Use for than one account to paint pixels",
  Va = () => "Usar mais de uma conta para pintar",
  Ha = (t = {}, e = {}) => (e.locale ?? o()) === "en" ? Ja() : Va(),
  za = t => `Your account have been banned. Reason: ${t.reason} (${t.description})`,
  Ya = t => `Sua conta foi banida. Motivo: ${t.reason} (${t.description})`,
  Za = (t, e = {}) => (e.locale ?? o()) === "en" ? za(t) : Ya(t),
  Wa = t => `Your account have been suspended until ${t.until}. Reason: ${t.reason} (${t.description})`,
...
class wt {
  ...
  async paint(e, a) {
    const n = lt(e, r => `t=(${r.tile[0]},${r.tile[1]}),s=${r.season}`),
      c = (await Promise.all(Object.values(n).map(r => {
        const [s, h] = r[0].tile, b = r[0].season, O = {
          colors: r.map(S => S.colorIdx),
          coords: r.flatMap(S => S.pixel),
          t: a
        };
        return this.request(`/s${b}/pixel/${s}/${h}`, {
          method: "POST",
    ...
    if (c.length) {
      ...
      if (r.status === 403) {
        ...
        const s = await c[0].json(),
          ...
          b = st[h],
          O = it[h],
          S = s == null ? void 0 : s.suspension;
        if (S === "ban") throw new Error(Za({
          description: O,
          reason: b
        }));
        if (S === "timeout") {
          const J = new Date(Date.now() + ((s == null ? void 0 : s.durationMs) ?? 0));
          throw new Error(Xa({
            description: O,
            reason: b,
  ...
  async createAlliance(e) {
    ...
    if (a.status === 400) {
      const n = await a.json();
      throw n.error === "max_characters" ? new Error(Ge()) : n.error === "name_taken" ? new Error(Ue()) : n.error == "empty_name" ? new Error(Fe()) : new Error(i())
    } else throw a.status === 403 ? new Error(Ve()) : new Error(i())
  }
  async leaveAlliance() {
  ...
  async updateAllianceHeadquarters(e, a) {
    ...
    if (n.status !== 200) throw n.status === 403 ? new Error(p()) : new Error(i())
  }
...
class bt {
  ...
  set(e, a) {
    ...
    if (n >= this.bytes.length) {
      ...
      for (let h = 0; h < this.bytes.length; h++) r[h + s] = this.bytes[h];
      this.bytes = r
    }
  ...
  constructor() {
    ...
    u(this, "create", e => {
      ...
      const {
        message: a,
        ...n
      } = e, l = typeof(e == null ? void 0 : e.id) == "number" || e.id && ((s = e.id) == null ? void 0 : s.length) > 0 ? e.id : $++, c = e.dismissable === void 0 ? !0 : e.dismissable, r = e.type === void 0 ? "default" : e.type;
      return U(() => {
