...
var ze = (n, e, t) => e in n ? Te(n, e, {
  enumerable: !0,
  configurable: !0,
  writable: !0,
  value: t
}) : n[e] = t;
var b = (n, e, t) => ze(n, typeof e != "symbol" ? e + "" : e, t);
import "./Bzak7iHL.js";
import {
  p as Ce,
  d as $,
...
var Fe = (() => {
  ...
  _e = g(be).replace(/bull/g, X).replace(/blockCode/g, /(?: {4}| {0,3}\t)/).replace(/fences/g, / {0,3}(?:`{3,}|~{3,})/).replace(/blockquote/g, / {0,3}>/).replace(/heading/g, / {0,3}#{1,6}/).replace(/html/g, / {0,3}<[^\n>]+>\n/).replace(/\|table/g, "").getRegex(),
  Be = g(be).replace(/bull/g, X).replace(/blockCode/g, /(?: {4}| {0,3}\t)/).replace(/fences/g, / {0,3}(?:`{3,}|~{3,})/).replace(/blockquote/g, / {0,3}>/).replace(/heading/g, / {0,3}#{1,6}/).replace(/html/g, / {0,3}<[^\n>]+>\n/).replace(/table/g, / {0,3}\|?(?:[:\- ]*\|)+[\:\- ]*\n/).getRegex(),
  ...
  ee = /<!--(?:-?>|[\s\S]*?(?:-->|$))/,
  Ze = g("^ {0,3}(?:<(script|pre|style|textarea)[\\s>][\\s\\S]*?(?:</\\1>[^\\n]*\\n+|$)|comment[^\\n]*(\\n+|$)|<\\?[\\s\\S]*?(?:\\?>\\n*|$)|<![A-Z][\\s\\S]*?(?:>\\n*|$)|<!\\[CDATA\\[[\\s\\S]*?(?:\\]\\]>\\n*|$)|</?(tag)(?: +|\\n|/?>)[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$)|<(?!script|pre|style|textarea)([a-z][\\w-]*)(?:attribute)*? */?>(?=[ \\t]*(?:\\n|$))[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$)|</(?!script|pre|style|textarea)[a-z][\\w-]*\\s*>(?=[ \\t]*(?:\\n|$))[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$))", "i").replace("comment", ee).replace("tag", N).replace("attribute", / +[a-zA-Z:_][\w.:-]*(?: *= *"[^"\n]*"| *= *'[^'\n]*'| *= *[^\s"'=<>`]+)?/).getRegex(),
  ke = g(K).replace("hr", I).replace("heading", " {0,3}#{1,6}(?:\\s|$)").replace("|lheading", "").replace("|table", "").replace("blockquote", " {0,3}>").replace("fences", " {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list", " {0,3}(?:[*+-]|1[.)])[ \\t]").replace("html", "</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag", N).getRegex(),
  ...
  Qe = {
    ...
    lheading: Be,
    table: ce,
    paragraph: g(K).replace("hr", I).replace("heading", " {0,3}#{1,6}(?:\\s|$)").replace("|lheading", "").replace("table", ce).replace("blockquote", " {0,3}>").replace("fences", " {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list", " {0,3}(?:[*+-]|1[.)])[ \\t]").replace("html", "</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag", N).getRegex()
  },
  Ge = {
    ...te,
    ...
    fences: S,
    lheading: /^(.+?)\n {0,3}(=+|-+) *(?:\n+|$)/,
    paragraph: g(K).replace("hr", I).replace("heading", ` *#{1,6} *[^
  ...
  W = /[\s\p{P}\p{S}]/u,
  ne = /[^\s\p{P}\p{S}]/u,
  Ke = g(/^((?![*_])punctSpace)/, "u").replace(/punctSpace/g, W).getRegex(),
  ye = /(?!~)[\p{P}\p{S}]/u,
  Je = /(?!~)[\s\p{P}\p{S}]/u,
  et = /(?:[^\s\p{P}\p{S}]|~)/u,
  tt = g(/link|precode-code|html/, "g").replace("link", /\[(?:[^\[\]`]|(?<a>`+)[^`]+\k<a>(?!`))*?\]\((?:\\[\s\S]|[^\\\(\)]|\((?:\\[\s\S]|[^\\\(\)])*\))*\)/).replace("precode-", Fe ? "(?<!`)()" : "(^^|[^`])").replace("code", /(?<b>`+)[^`]+\k<b>(?!`)/).replace("html", /<(?! )[^<>]*?>/).getRegex(),
  xe = /^(?:\*+(?:((?!\*)punct)|([^\s*]))?)|^_+(?:((?!_)punct)|([^\s_]))?/,
  nt = g(xe, "u").replace(/punct/g, z).getRegex(),
  rt = g(xe, "u").replace(/punct/g, ye).getRegex(),
  ve = "^[^_*]*?__[^_*]*?\\*[^_*]*?(?=__)|[^*]+(?=[^*])|(?!\\*)punct(\\*+)(?=[\\s]|$)|notPunctSpace(\\*+)(?!\\*)(?=punctSpace|$)|(?!\\*)punctSpace(\\*+)(?=notPunctSpace)|[\\s](\\*+)(?!\\*)(?=punct)|(?!\\*)punct(\\*+)(?!\\*)(?=punct)|notPunctSpace(\\*+)(?=notPunctSpace)",
  at = g(ve, "gu").replace(/notPunctSpace/g, ne).replace(/punctSpace/g, W).replace(/punct/g, z).getRegex(),
  st = g(ve, "gu").replace(/notPunctSpace/g, et).replace(/punctSpace/g, Je).replace(/punct/g, ye).getRegex(),
  it = g("^[^_*]*?\\*\\*[^_*]*?_[^_*]*?(?=\\*\\*)|[^_]+(?=[^_])|(?!_)punct(_+)(?=[\\s]|$)|notPunctSpace(_+)(?!_)(?=punctSpace|$)|(?!_)punctSpace(_+)(?=notPunctSpace)|[\\s](_+)(?!_)(?=punct)|(?!_)punct(_+)(?!_)(?=punct)", "gu").replace(/notPunctSpace/g, ne).replace(/punctSpace/g, W).replace(/punct/g, z).getRegex(),
  ot = g(/^~~?(?:((?!~)punct)|[^\s~])/, "u").replace(/punct/g, z).getRegex(),
  lt = "^[^~]+(?=[^~])|(?!~)punct(~~?)(?=[\\s]|$)|notPunctSpace(~~?)(?!~)(?=punctSpace|$)|(?!~)punctSpace(~~?)(?=notPunctSpace)|[\\s](~~?)(?!~)(?=punct)|(?!~)punct(~~?)(?!~)(?=punct)|notPunctSpace(~~?)(?=notPunctSpace)",
  ct = g(lt, "gu").replace(/notPunctSpace/g, ne).replace(/punctSpace/g, W).replace(/punct/g, z).getRegex(),
  pt = g(/\\(punct)/, "gu").replace(/punct/g, z).getRegex(),
  ht = g(/^<(scheme:[^\s\x00-\x1f<>]*|email)>/).replace("scheme", /[a-zA-Z][a-zA-Z0-9+.-]{1,31}/).replace("email", /[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+(@)[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)+(?![-_])/).getRegex(),
  ut = g(ee).replace("(?:-->|$)", "-->").getRegex(),
  dt = g("^comment|^</[a-zA-Z][\\w:-]*\\s*>|^<[a-zA-Z][\\w-]*(?:attribute)*?\\s*/?>|^<\\?[\\s\\S]*?\\?>|^<![a-zA-Z]+\\s[\\s\\S]*?>|^<!\\[CDATA\\[[\\s\\S]*?\\]\\]>").replace("comment", ut).replace("attribute", /\s+[a-zA-Z:_][\w.:-]*(?:\s*=\s*"[^"]*"|\s*=\s*'[^']*'|\s*=\s*[^\s"'=<>`]+)?/).getRegex(),
  D = /(?:\[(?:\\[\s\S]|[^\[\]\\])*\]|\\[\s\S]|`+(?!`)[^`]*?`+(?!`)|``+(?=\])|[^\[\]\\`])*?/,
  mt = g(/^!?\[(label)\]\(\s*(href)(?:(?:[ \t]+(?:\n[ \t]*)?|\n[ \t]*)(title))?\s*\)/).replace("label", D).replace("href", /<(?:\\.|[^\n<>\\])+>|[^ \t\n\x00-\x1f]*/).replace("title", /"(?:\\"?|[^"\\])*"|'(?:\\'?|[^'\\])*'|\((?:\\\)?|[^)\\])*\)/).getRegex(),
  Se = g(/^!?\[(label)\]\[(ref)\]/).replace("label", D).replace("ref", J).getRegex(),
  Ae = g(/^!?\[(ref)\](?:\[\])?/).replace("ref", J).getRegex(),
  gt = g("reflink|nolink(?!\\()", "g").replace("reflink", Se).replace("nolink", Ae).getRegex(),
  ...
  ft = {
    ...re,
    link: g(/^!?\[(label)\]\((.*?)\)/).replace("label", D).getRegex(),
    reflink: g(/^!?\[(label)\]\s*\[([^\]]*)\]/).replace("label", D).getRegex()
  },
  Q = {
    ...re,
    ...
    delLDelim: ot,
    delRDelim: ct,
    url: g(/^((?:protocol):\/\/|www\.)(?:[a-zA-Z0-9\-]+\.?)+[^\s<]*|^email/).replace("protocol", pe).replace("email", /[A-Za-z0-9._+-]+(@)[a-zA-Z0-9-_]+(?:\.[a-zA-Z0-9-_]*[a-zA-Z0-9])+(?![-_])/).getRegex(),
...
`),
              m = this.blockquote(_);
            i[i.length - 1] = m, a = a.substring(0, a.length - c.raw.length) + m.raw, r = r.substring(0, r.length - c.text.length) + m.text;
            break
...
`),
              m = this.list(_);
            i[i.length - 1] = m, a = a.substring(0, a.length - u.raw.length) + m.raw, r = r.substring(0, r.length - c.raw.length) + m.raw, t = _.substring(i.at(-1).raw.length).split(`
`);
            continue
          }
        }
...
`, 1)[0],
            m = !c.trim(),
            k = 0;
          if (this.options.pedantic ? (k = 2, u = c.trimStart()) : m ? k = e[1].length + 1 : (k = c.search(this.rules.other.nonSpaceChar), k = k > 4 ? 1 : k, u = c.slice(k), k += e[1].length), m && this.rules.other.blankLine.test(_) && (d += _ + `
`, n = n.substring(_.length + 1), h = !0), !h) {
            let R = this.rules.other.nextBulletRegex(k),
              F = this.rules.other.hrRegex(k),
              se = this.rules.other.fencesBeginRegex(k),
              ie = this.rules.other.headingBeginRegex(k),
              Pe = this.rules.other.htmlBeginRegex(k),
              Re = this.rules.other.blockquoteBeginRegex(k);
            for (; n;) {
              let Z = n.split(`
`, 1)[0],
                C;
              if (_ = Z, this.options.pedantic ? (_ = _.replace(this.rules.other.listReplaceNesting, "  "), C = _) : C = _.replace(this.rules.other.tabCharGlobal, "    "), se.test(_) || ie.test(_) || Pe.test(_) || Re.test(_) || R.test(_) || F.test(_)) break;
              if (C.search(this.rules.other.nonSpaceChar) >= k || !_.trim()) u += `
` + C.slice(k);
              else {
                if (m || c.replace(this.rules.other.tabCharGlobal, "    ").search(this.rules.other.nonSpaceChar) >= 4 || se.test(c) || ie.test(c) || F.test(c)) break;
                u += `
` + _
              }
              m = !_.trim(), d += Z + `
`, n = n.substring(Z.length + 1), c = C.slice(k)
            }
          }
...
` ? e[1].slice(0, -1) : e[1];
    ...
    link(n) {
      ...
      if (e) {
        ...
        return a = a.trim(), this.rules.other.startAngleBracket.test(a) && (this.options.pedantic && !this.rules.other.endAngleBracket.test(t) ? a = a.slice(1) : a = a.slice(1, -1)), me(e, {
    ...
    reflink(n, e) {
      ...
      if ((t = this.rules.inline.reflink.exec(n)) || (t = this.rules.inline.nolink.exec(n))) {
        ...
        return me(t, r, t[0], this.lexer, this.rules)
      }
    }
    emStrong(n, e, t = "") {
  ...
  y = class G {
    constructor(e) {
      ...
      b(this, "inlineQueue");
      b(this, "tokenizer");
      this.tokens = [], this.tokens.links = Object.create(null), this.options = e || P, this.options.tokenizer = this.options.tokenizer || new B, this.tokenizer = this.options.tokenizer, this.tokenizer.options = this.options, this.tokenizer.lexer = this, this.inlineQueue = [], this.state = {
        inLink: !1,
        inRawBlock: !1,
        top: !0
      };
      ...
      this.options.pedantic ? (t.block = E.pedantic, t.inline = O.pedantic) : this.options.gfm && (t.block = E.gfm, this.options.breaks ? t.inline = O.breaks : t.inline = O.gfm), this.tokenizer.rules = t
    }
    static get rules() {
      return {
        block: E,
...
` + s.text, this.inlineQueue.pop(), this.inlineQueue.at(-1).src = l.text) : t.push(s);
    ...
    inlineTokens(e, t = []) {
      ...
      for (; e;) {
        o || (s = ""), o = !1;
        let c;
        if ((d = (h = this.options.extensions) == null ? void 0 : h.inline) != null && d.some(m => (c = m.call({
            lexer: this
        ...
        if ((u = this.options.extensions) != null && u.startInline) {
          ...
          this.options.extensions.startInline.forEach(F => {
            R = F.call({
              lexer: this
            }, k), typeof R == "number" && R >= 0 && (m = Math.min(m, R))
          }), m < 1 / 0 && m >= 0 && (_ = e.substring(0, m + 1))
        }
...
`
  ...
  x = class Y {
    constructor(e) {
      ...
      b(this, "renderer");
      b(this, "textRenderer");
      this.options = e || P, this.options.renderer = this.options.renderer || new H, this.renderer = this.options.renderer, this.renderer.options = this.options, this.renderer.parser = this, this.textRenderer = new ae
    }
    static parse(e, t) {
  ...
  xt = class {
    ...
    use(...n) {
      ...
      return n.forEach(t => {
        ...
        if (t.tokenizer) {
          let r = this.defaults.tokenizer || new B(this.defaults);
        ...
        if (t.hooks) {
          ...
          for (let i in t.hooks) {
            ...
            let o = i,
              s = t.hooks[o],
              p = r[o];
            M.passThroughHooks.has(i) ? r[o] = l => {
...
f.options = f.setOptions = function(n) {
  return A.setOptions(n), f.defaults = A.defaults, fe(f.defaults), f
};
f.getDefaults = V;
f.defaults = P;
f.use = function(...n) {
  return A.use(...n), f.defaults = A.defaults, fe(f.defaults), f
};
...
`,
  ...
  At = `We have just launched a new update focused on **personalization**!
...
`,
  ...
  Rt = `This update brings several improvements and fixes for a smoother experience:
...
`,
  ...
  zt = `This patch focuses on interface stability across devices and a better moderation workflow:
...
`,
  ...
  $t = `Import any image and position it over the map as a painting guide! Access overlays from the **new button in the main sidebar**.
...
`,
  ...
  qt = `With v1.3.0, we’re reducing the moderation intervention and giving more freedom to the players to better match Wplace’s core idea: a living canvas shaped by the community.
...
`,
  ...
  Sn = Object.assign({
    ...
    "./markdown/1.1.2 - 🧰 UI Fixes & Moderation Improvements.md": Ct,
    "./markdown/1.2.0 - 🖼️ Pixel Overlays & Achievement Frames.md": Ot,
    "./markdown/1.3.0 - 🛠️ Moderation Adjustments, Hotspots and Improved Anti-Cheat System.md": Mt,
    "./markdown/1.3.1 - 🛠️  Hotfixes and Opt-out from Hotspots copy.md": Ft,
    "./markdown/1.3.2 - 🤖 Anti-cheat improvements and Google Drive overlay sync.md": Et,
    "./markdown/1.3.3 - ⚽ World Cup Predictions and Cosmetics & Anti-cheat Improvements.md": Dt,
    "./markdown/1.4.0 - 🛡️ The New Alliance System.md": Ht,
    "./markdown/1.4.1 - 🛡️ Alliance System Fixes.md": Wt,
    "./markdown/1.4.10 - 🛠️ Mobile Overlay Rendering.md": Ut,
    "./markdown/1.4.11 - 🛠️ Phone Verification Fixes.md": Gt,
    "./markdown/1.4.12 - 🛠️ Alliance Overlay Reliability.md": Vt,
    "./markdown/1.4.2 - 🛠️ Headquarters Preview Reliability.md": Kt,
    "./markdown/1.4.3 - 🛠️ Alliance Usability Fixes.md": en,
    "./markdown/1.4.4 - 🧭 Alliance Canvas Overlays.md": nn,
    "./markdown/1.4.5 - 🎨 Alliance Canvas Tools and Map Display.md": an,
    "./markdown/1.4.6 - 🛠️ Map Interaction Reliability.md": on,
    "./markdown/1.4.7 - 🖼️ Alliance Overlays and Asset Studio.md": cn,
    "./markdown/1.4.8 - 🛠️ Overlay Reliability.md": hn,
    "./markdown/1.4.9 - 🛍️ Mobile Overlay Fixes.md": dn,
...
function jn(n, e) {
  ...
  T(l);
  var d = j(l, 2);
  qe(d, () => e.patchNote.html, !0), T(d), T(a), oe(() => {
    U(o, `v${e.patchNote.version??""}`), U(h, e.patchNote.title)
  }), le(n, a), $e()
}
export {
