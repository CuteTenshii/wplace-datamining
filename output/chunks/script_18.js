import {
  P as ee,
  m as se,
  Q as ae,
  R as q,
  k as N,
  T as ue,
  l as de,
  j as J,
  U as ve,
  V as oe,
  W as _e,
  X as K,
  Y as U,
  Z as S,
  _ as ce,
  a0 as he,
  a1 as F,
  a2 as be,
  a3 as O,
  a4 as L,
  v as pe,
  a5 as $,
  a6 as Ee,
  a7 as j,
  a8 as re,
  a9 as me,
  aa as ne,
  ab as Te,
  ac as we,
  ad as X,
  D as Ie,
  ae as fe,
  af as Ae,
  ag as ge,
  ah as ye,
  ai as De,
  aj as Ne,
  ak as xe
} from "./DHzwV35n.js";
(function() {
  try {
    var f = typeof window < "u" ? window : typeof global < "u" ? global : typeof globalThis < "u" ? globalThis : typeof self < "u" ? self : {};
    f.SENTRY_RELEASE = {
      id: "dfb4bddd9fb2b4748eaf19466331b7b547c362b7"
    }
  } catch {}
})();
try {
  (function() {
    var f = typeof window < "u" ? window : typeof global < "u" ? global : typeof globalThis < "u" ? globalThis : typeof self < "u" ? self : {},
      r = new f.Error().stack;
    r && (f._sentryDebugIds = f._sentryDebugIds || {}, f._sentryDebugIds[r] = "24b0a935-c76c-4b16-87b6-55e6afb6152f", f._sentryDebugIdIdentifier = "sentry-dbid-24b0a935-c76c-4b16-87b6-55e6afb6152f")
  })()
} catch {}
let k = null;

function Me(f, r) {
  return r
}

function Ce(f, r, e) {
  for (var s = f.items, u = [], d = r.length, t = 0; t < d; t++) ye(r[t].e, u, !0);
  var c = d > 0 && u.length === 0 && e !== null;
  if (c) {
    var m = e.parentNode;
    De(m), m.append(e), s.clear(), A(f, r[0].prev, r[d - 1].next)
  }
  Ne(u, () => {
    for (var E = 0; E < d; E++) {
      var o = r[E];
      c || (s.delete(o.k), A(f, o.prev, o.next)), fe(o.e, !c)
    }
  })
}

function He(f, r, e, s, u, d = null) {
  var t = f,
    c = {
      flags: r,
      items: new Map,
      first: null
    },
    m = (r & ae) !== 0;
  if (m) {
    var E = f;
    t = N ? q(ue(E)) : E.appendChild(ee())
  }
  N && de();
  var o = null,
    y = !1,
    T = new Map,
    x = ve(() => {
      var v = e();
      return me(v) ? v : v == null ? [] : re(v)
    }),
    i, b;

  function n() {
    Re(b, i, c, T, t, u, r, s, e), d !== null && (i.length === 0 ? o ? ne(o) : o = F(() => d(t)) : o !== null && Te(o, () => {
      o = null
    }))
  }
  se(() => {
    b ?? (b = xe), i = J(x);
    var v = i.length;
    if (y && v === 0) return;
    y = v === 0;
    let p = !1;
    if (N) {
      var w = oe(t) === _e;
      w !== (v === 0) && (t = K(), q(t), U(!1), p = !0)
    }
    if (N) {
      for (var g = null, _, a = 0; a < v; a++) {
        if (S.nodeType === ce && S.data === he) {
          t = S, p = !0, U(!1);
          break
        }
        var l = i[a],
          h = s(l, a);
        _ = P(S, c, g, null, l, h, a, u, r, e), c.items.set(h, _), g = _
      }
      v > 0 && q(K())
    }
    if (N) v === 0 && d && (o = F(() => d(t)));
    else if (be()) {
      var C = new Set,
        M = pe;
      for (a = 0; a < v; a += 1) {
        l = i[a], h = s(l, a);
        var D = c.items.get(h) ?? T.get(h);
        D ? (r & (O | L)) !== 0 && ie(D, l, a, r) : (_ = P(null, c, null, null, l, h, a, u, r, e, !0), T.set(h, _)), C.add(h)
      }
      for (const [I, H] of c.items) C.has(I) || M.skipped_effects.add(H.e);
      M.oncommit(n)
    } else n();
    p && U(!0), J(x)
  }), N && (t = S)
}

function Re(f, r, e, s, u, d, t, c, m) {
  var Q, W, Z, z;
  var E = (t & Ae) !== 0,
    o = (t & (O | L)) !== 0,
    y = r.length,
    T = e.items,
    x = e.first,
    i = x,
    b, n = null,
    v, p = [],
    w = [],
    g, _, a, l;
  if (E)
    for (l = 0; l < y; l += 1) g = r[l], _ = c(g, l), a = T.get(_), a !== void 0 && ((Q = a.a) == null || Q.measure(), (v ?? (v = new Set)).add(a));
  for (l = 0; l < y; l += 1) {
    if (g = r[l], _ = c(g, l), a = T.get(_), a === void 0) {
      var h = s.get(_);
      if (h !== void 0) {
        s.delete(_), T.set(_, h);
        var C = n ? n.next : i;
        A(e, n, h), A(e, h, C), B(h, C, u), n = h
      } else {
        var M = i ? i.e.nodes_start : u;
        n = P(M, e, n, n === null ? e.first : n.next, g, _, l, d, t, m)
      }
      T.set(_, n), p = [], w = [], i = n.next;
      continue
    }
    if (o && ie(a, g, l, t), (a.e.f & X) !== 0 && (ne(a.e), E && ((W = a.a) == null || W.unfix(), (v ?? (v = new Set)).delete(a))), a !== i) {
      if (b !== void 0 && b.has(a)) {
        if (p.length < w.length) {
          var D = w[0],
            I;
          n = D.prev;
          var H = p[0],
            Y = p[p.length - 1];
          for (I = 0; I < p.length; I += 1) B(p[I], D, u);
          for (I = 0; I < w.length; I += 1) b.delete(w[I]);
          A(e, H.prev, Y.next), A(e, n, H), A(e, Y, D), i = D, n = Y, l -= 1, p = [], w = []
        } else b.delete(a), B(a, i, u), A(e, a.prev, a.next), A(e, a, n === null ? e.first : n.next), A(e, n, a), n = a;
        continue
      }
      for (p = [], w = []; i !== null && i.k !== _;)(i.e.f & X) === 0 && (b ?? (b = new Set)).add(i), w.push(i), i = i.next;
      if (i === null) continue;
      a = i
    }
    p.push(a), n = a, i = a.next
  }
  if (i !== null || b !== void 0) {
    for (var R = b === void 0 ? [] : re(b); i !== null;)(i.e.f & X) === 0 && R.push(i), i = i.next;
    var V = R.length;
    if (V > 0) {
      var le = (t & ae) !== 0 && y === 0 ? u : null;
      if (E) {
        for (l = 0; l < V; l += 1)(Z = R[l].a) == null || Z.measure();
        for (l = 0; l < V; l += 1)(z = R[l].a) == null || z.fix()
      }
      Ce(e, R, le)
    }
  }
  E && Ie(() => {
    var G;
    if (v !== void 0)
      for (a of v)(G = a.a) == null || G.apply()
  }), f.first = e.first && e.first.e, f.last = n && n.e;
  for (var te of s.values()) fe(te.e);
  s.clear()
}

function ie(f, r, e, s) {
  (s & O) !== 0 && $(f.v, r), (s & L) !== 0 ? $(f.i, e) : f.i = e
}

function P(f, r, e, s, u, d, t, c, m, E, o) {
  var y = k,
    T = (m & O) !== 0,
    x = (m & we) === 0,
    i = T ? x ? Ee(u, !1, !1) : j(u) : u,
    b = (m & L) === 0 ? t : j(t),
    n = {
      i: b,
      v: i,
      k: d,
      a: null,
      e: null,
      prev: e,
      next: s
    };
  k = n;
  try {
    if (f === null) {
      var v = document.createDocumentFragment();
      v.append(f = ee())
    }
    return n.e = F(() => c(f, i, b, E), N), n.e.prev = e && e.e, n.e.next = s && s.e, e === null ? o || (r.first = n) : (e.next = n, e.e.next = n.e), s !== null && (s.prev = n, s.e.prev = n.e), n
  } finally {
    k = y
  }
}

function B(f, r, e) {
  for (var s = f.next ? f.next.e.nodes_start : e, u = r ? r.e.nodes_start : e, d = f.e.nodes_start; d !== null && d !== s;) {
    var t = ge(d);
    u.before(d), d = t
  }
}

function A(f, r, e) {
  r === null ? f.first = e : (r.next = e, r.e.next = e && e.e), e !== null && (e.prev = r, e.e.prev = r && r.e)
}
export {
  k as c, He as e, Me as i
};