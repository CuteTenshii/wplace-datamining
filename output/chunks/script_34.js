...
function m(e, {
  ...
  const n = getComputedStyle(e),
    ...
    b = parseFloat(n[`margin${d[1]}`]),
    g = parseFloat(n[`border${d[0]}Width`]),
    _ = parseFloat(n[`border${d[1]}Width`]);
  return {
    delay: t,
    duration: i,
    easing: s,
    css: a => `overflow: hidden;opacity: ${Math.min(a*20,1)*c};${p}: ${a*l}px;padding-${o[0]}: ${a*f}px;padding-${o[1]}: ${a*y}px;margin-${o[0]}: ${a*u}px;margin-${o[1]}: ${a*b}px;border-${o[0]}-width: ${a*g}px;border-${o[1]}-width: ${a*_}px;min-${p}: 0`
  }
}
export {
  w as f, m as s
